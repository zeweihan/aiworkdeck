...
2026-09-24 起多三条：PATCH 5（`mqc-timeline-master/scripts/render_multiband.py`
的三泳道横向重叠）、PATCH 6（`export_pptx.py` 纵向裱框后标题被带 `data-role="canvas-bg"`
标记的背景矩形收养，认标记修）与 PATCH 7（同一函数再加包含关系判定，连没有标记的
日期型/期间型/分页内容底也认得出来，dev-board#893），都是**修上游的 bug**，上游修好就能删。
...
## PATCH 7 · pptx 导出的背景判定改用包含关系（`scripts/export_pptx.py` · `attach_text()`）
