...
| `sheet_read_range(range, sheet, withFormat?)` | Read cell values in a range (text as strings, numeric/formula results as numbers, formula strings listed separately); omit range to read the whole used range; withFormat=true also returns a row-grouped format summary (font/size/alignment/number format/borders/fill, only items that differ from the default) |
| `sheet_write_cells(startCell, rowsJson, sheet, inheritFormat?)` | Batch-write a JSON 2D array from a start cell; numbers land as values, `"=SUM(B2:B5)"` lands as a formula, everything else as text; rows appended below a table automatically inherit the previous row's format (reported as formatInherited), inheritFormat=false turns this off |
...
Spreadsheet essentials: the sheet parameter is a worksheet name or index (0-based); omitted = the active worksheet; ranges always use the `A1:D20` form; **xlsx has no track-changes mode - writes take effect immediately**; if you make a mistake use `doc_undo` (the system took a document snapshot before your first edit; last resort `doc_restore_checkpoint()`); write data before formatting (first `sheet_write_cells`, then in the same turn `sheet_format_cells` / `sheet_set_borders` / `sheet_set_row_col`). **New rows/columns must match the formatting of the adjacent existing content** (font, size, alignment, number format, borders, fill) - never leave cells that visibly look added afterwards: when appending below a table, `sheet_write_cells` inherits the previous row's format automatically; confirm via `formatInherited` in its result; in every other case (writing the first row under the header, adding a column, writing into a row inserted mid-table with `sheet_edit_rows_cols`, or no `formatInherited` in the result) first read the neighbouring rows/columns with `sheet_read_range(withFormat=true)`, then align with `sheet_format_cells` / `sheet_set_borders`.
