...
- **项目下拉的远程设备分组**（dev-board#250）：`refreshProjects` 顺带拉 `fetchMobileDevices`（`GET /api/mobile/devices`，见 mobile-sync.md），按设备渲染 `<optgroup>`，远程项目 option 的 value 是哨兵 `remote::<deviceId>::<key>`。`onProjectSelect` 拦截该前缀：**不切换 projectId**，把下拉显示值退回当前本地项目（写法照抄 `__new__` 哨兵），弹一条 4 秒自隐提示（`.remote-hint`）讲边界——远程设备项目只读，只能是跨设备文件传输的来源/目标。#251（跨设备文件传输）会把这个拦截改成打开传输面板。
