...
- **新增 office_* 工具四件套**：后端 OfficeEditTools 加 @Tool + officeExecutor.js 的 HANDLERS 加实现 + **wpsWordHandlers/wpsEtHandlers/wpsWppHandlers 对应那一面也加实现** + COMMAND_DISPLAY_NAMES 加中文名，**并在 COMMAND_HOSTS 标宿主**。WPS 文字会话的 officeHost 同样是 `word`，所以只在 Office 面实现的 Word 工具会让 WPS 用户拿到 `unsupported command`。没有客户端实现的远端工具 = 30s 超时空转（PptxEditTools 教训）。**同一条教训的另一面（dev-board#799）**：无前缀工具里有一批的收尾要桌面 LOWA 前端配合（发 open_file / reload_file / text_reload_file / ppt_config），它们在任务窗格会话里恒可见却是纯空转——`pptx_generate` 还会回一句「已唤起 PPT 生成配置界面…等待用户操作」，而那个界面在插件里根本不存在，模型就此停住、最终告诉用户「请在弹出的界面里选择」。现在这类工具用 `@ToolMeta(requiresHost = LOWA)` 自己声明，任务窗格会话既不下发规格也分发不到；**只读面没动**（`pdf_list_files` / `pdf_inspect` / `pptx_inspect_format` / `pptx_list_files` 照常可见）。清单与绊线在 `ToolDeclarationContractTest`。**工具名前缀决定宿主可见性**：Excel 面必须 office_excel_*、PPT 面必须 office_ppt_*，其余 office_* 归 Word——起错前缀会漏进错误宿主的会话（死路径）。
