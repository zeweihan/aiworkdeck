...
- `office-addin/taskpane/` — Vue3 源码：`App.vue`（视图切换+项目下拉；header 不再自绘品牌名——Office 按 manifest DisplayName 已画一条标题，2026-08 去重）、`components/SettingsView.vue`（主路径=账户登录。**`mode` 是三态**（dev-board#766 起）：`phone` / `email` 两条验证码路 + `password`（两站存量口令账号，靠「改用口令登录 / 改用验证码登录」一行常驻文字链切换）——**口令那条链子必须留在所有 v-if 分支之外**，挂在邮箱 tab 里的话大陆站隐掉该 tab 会把存量口令用户的入口一起埋掉；60 秒重发倒计时两个渠道共用；**人机验证控件的 `#login-captcha` 与 `-trigger` 两个节点必须留在 v-if 分支之外**——控件在 onMounted 时按 id 认下节点并一直持有，放进分支里用户切一次 tab 节点就被拆掉，滑块弹不出来也取不到 token，而且一声不响；后端地址、awdk_ Key、awdt_ 设备令牌三项一起收进「高级设置」折叠区。**站点能力决定 tab**（dev-board#766，取代了原先「插件判不了站点、两个 tab 都给用户挑」的做法）：云后端在 `captcha-config` 里带回 `accountIdentity`，判定与渲染分在 `lib/loginTabs.js` 与组件两处，详见下方契约节）、`components/ChatView.vue`（纯渲染+交互层；会话态全在 `lib/chatSession.js` 模块级 store——messages/conversationId/SSE 连接/streaming/草稿，切视图不卸载会话）、`lib/`（settings/api/sse/wordDoc/officeExecutor/chatSession/minimalEdit/captcha/loginTabs）。会话持久化：conversationId 按项目落 localStorage（`awd_addin_conv_{projectId}`），窗格重建时经 `GET /api/ai/history` 回灌（tag parser 拆 thinking/final，工具 chip 不回灌），回灌建连的首个 run_state 是权威状态（RUNNING 则锁输入续写，restorePending 标记位区分于 send 建连语义）。**2026-08-25 批次二（PR#597）**：计划审批卡（解析器新增 onArtifact，<artifact> 整块进消息卡）；引用定位（officeExecutor.locateInDocument 客户端直连 search+select，「」/“” 引文 6-80 字符成可点 chip）；批注队列/总结/校对空态快捷入口；附件选择器（contextItems 后端既有字段按 fileId 读内容）；会话删除/重命名（后端新端点 DELETE /api/ai/conversation/{id} 与 POST .../title，RUNNING 拒删 409、标题 1-60）；**中英双语**（lib/i18n.js 平铺 215 键，Office displayLanguage 判定，模板裸中文由 i18n.test.js 静态扫描钉死；模型可见的指令性错误文案保持中文不翻）。**部署注意**：云后端 jar 本地打包必须带 `-Djavacpp.platform=linux-x86_64`（否则 1GB 全平台包）；scp 大 jar 必须 sha256 对账后再换入（2026-08-25 北京实测传输损坏起不来）；**SG 服务已钉 /usr/lib/jvm/jre-21/bin/java**（此前 /usr/bin/java 是 17，plugin-api 刻意 release=21，动态插件类在 SG 一直静默加载失败）。
...
- `office-addin/taskpane/lib/excelFormat.js` — **Excel 两族的格式判定层**（dev-board#844，与 `excelReplace.js` 同一类角色）：`planFormatInheritance`（`excel_set_values` 哪几行先沿用上一行格式）、`normalizeOfficeCellProps` / `normalizeEtCell`（两个宿主的逐格格式归一成同一套字段）、`summarizeCellFormats`（`excel_get_range(withFormat)` 压成 `baseFont` + `columns` 列级摘要 + `cells` 例外格）。宿主相关的读写各在执行器里（Office：`inheritOfficeRowFormats` / `readOfficeCellFormats`；WPS：`inheritEtRowFormats` / `copyEtCellFormat` / `readEtCellFormat`）。字段名 `fontName/fontSize/bold/italic/color/background/hAlign/vAlign/numberFormat/borders/wrap` 与桌面端 `sheet_read_range` 对齐。
...
- **登录页按站点给 tab（`accountIdentity`，dev-board#766）**：官网两个站各只认一种账号本体（大陆站手机号、国际站邮箱，见官网 `lib/phone-policy.ts` / `lib/mail-policy.ts`「两个都为真的站不存在，两个都为假也不存在」）。`GET /api/auth/account-login/captcha-config` 的载荷里多了一个 **`accountIdentity`（`phone` | `email`）**，由 `AwdkLoginService.accountIdentity()` 按`ai.account.base-url` 指向的站点派生，**复用 `OfficialCloudEndpoint.isInternationalSite`那份按主机的判定**（`contains("workdeck.ai")` 会把 `workdeck.ai.evil.com` 算成国际站）。客户端侧的判定与 tab 计算全在 `taskpane/lib/loginTabs.js`（纯函数，组件只负责渲染）。三条口径：
  - **认不出一律当未知**（空串），未知时两个验证码 tab 都给——插件连的是用户自填的服务器地址，很可能是没有这个字段的旧云后端；猜错会把另一种账号的用户直接挡在门外（同 `visionCapability` 的「三态，undefined 不等于 false」）。官网不可达时 `provider` 为空但 `accountIdentity` 照给：站点只取决于本机配置，与这次出站成败无关。
  - **口令那条路不随站点隐藏**：站点只决定验证码走手机号还是邮箱，口令是两站存量账号共用的第三条路。
  - **当前 tab 一旦在新站点下不可用就立刻纠正**（`applyIdentity`），不看用户手动点过没有——那个 tab 马上就要从界面上消失（配置是异步到的，在那一两秒里用户完全可能已经点过一下）。
- **窗格底部的许可告示默认折叠成一行**（dev-board#765，AGPL §0 告示 + CLAUDE.md 溯源红线）：折叠行是「AI WorkDeck · AGPL-3.0 · 源代码 · 许可证 · 商标说明」，点开才是完整版文与三条链接，**内容一字未删**。三条不许动的地方：① 整块仍在 `#app` 之外的静态 HTML 里（Vue 挂不上时告示仍在），② 折叠用原生 `<details>`、**不接任何 JS**（拿 JS 管开合等于把上一条退化成「告示在、但永远展不开」），③ 中英两份同时写在页里，由 `<html lang>` + 纯 CSS 二选一（`i18n.js` 的 `syncDocumentLang()` 在模块加载与`setLang()` 时同步该属性）——页脚在 Vue 树外拿不到 `t()`，`:key` 重挂载也带不动它。`taskpane.html` 与 `taskpane-wps.html` 两份页脚必须逐字相同，回归 `legalNotice.test.js` 钉着。
...
- **ask_user 提问卡（dev-board#868）**：后端把问题以 `<question kind="ask_user" id header multi><option description>` 文本标记流出并落库，再发 SSE `ask_user` 事件；窗格解析器把 ask_user 的问题正文收进卡片（`components/AskUserCard.vue`）而不进气泡，事件到达时整块覆盖标记拼出的那份；回答经 `chatSession.answerAskUser()` 以 `<ask_user_answer id>` 结构化格式发送并带 `displayText`（用户气泡只显所选）。**`taskpane/lib/askUser.js` 与桌面端 `frontend/src/utils/askUserAnswer.mjs`、后端 `AskUserQuestion` 三处格式必须一起改**（`askUser.test.js` 有逐项对拍用例钉着）。不带 `kind` 的旧 `<question>` 行为不变。
...
- **`office_get_text` 是分页的**（dev-board#806，审计 B-05）：参数 `startChar`（0 开始，缺省 0）/ `maxChars`
  （缺省 50,000，上限 80,000），返回 `{text, startChar, returned, totalChars, truncated, nextStart?}`——
  **`nextStart` 有值就是「还没读完，用它当下一次的 startChar」，没有就是读到文末**。切法与上限的单源是
  `taskpane/lib/textPaging.js`（Office 面与 WPS 文字面共用一份：后端只有一条工具描述，两族不能对模型说两套话），
  后端 `OfficeEditTools` 也夹同一道上限。两个数的来历：80,000 = `ToolFileGuard.MAX_TOOL_TEXT_CHARS`
  （「单条工具结果交给模型」的既有口径，此前一处都没覆盖 office_*，插件侧是它的 2.5 倍）；
  **刻意不复用 `officeExecutor.MAX_TEXT_CHARS`（20 万）**——那个常量同时服务「随消息附带的内联正文上限」
  （对齐 `ContextAssemblerService.MAX_INLINE_CONTENT_CHARS`），动它会连带改掉内联正文口径。
  **`OfficePassChunker` 不受影响**：它切的是 `InlineContentCache` 里模型看到的那份正文，从不调 get_text。
...
- **新增 office_* 工具有 CI 对拍**（dev-board#806）：`node scripts/check-tool-parity.mjs` 把两族按「同一件事」配对，
  **对两族全覆盖**——新加一个编辑工具而没在 MATRIX 里配对、也没进 `LOWA_ONLY`/`OFFICE_ONLY`（要写理由）就转红，
  逼着加工具的人回答「另一族要不要跟着加」。同一个脚本还查每个 skill 的 `allowed_tools`：一族列了、另一族没列即失败。
- **新增 office_* 工具四件套**：后端 OfficeEditTools 加 @Tool + officeExecutor.js 的 HANDLERS 加实现 + **wpsWordHandlers/wpsEtHandlers/wpsWppHandlers 对应那一面也加实现** + COMMAND_DISPLAY_NAMES 加中文名，**并在 COMMAND_HOSTS 标宿主**。WPS 文字会话的 officeHost 同样是 `word`，所以只在 Office 面实现的 Word 工具会让 WPS 用户拿到 `unsupported command`。没有客户端实现的远端工具 = 30s 超时空转（PptxEditTools 教训）。**「一次调用做 N 件事」的命令还要进 `OfficeBridgeService.ACTION_TIMEOUT_SECONDS`**（`replace_batch` / `apply_standard_format` / `excel_replace`）：平超时的症状不是报错，而是后端先放弃、模型重发一次把同一批改动做两遍。**同一条教训的另一面（dev-board#799）**：无前缀工具里有一批的收尾要桌面 LOWA 前端配合（发 open_file / reload_file / text_reload_file / ppt_config），它们在任务窗格会话里恒可见却是纯空转——`pptx_generate` 还会回一句「已唤起 PPT 生成配置界面…等待用户操作」，而那个界面在插件里根本不存在，模型就此停住、最终告诉用户「请在弹出的界面里选择」。现在这类工具用 `@ToolMeta(requiresHost = LOWA)` 自己声明，任务窗格会话既不下发规格也分发不到；**只读面没动**（`pdf_list_files` / `pdf_inspect` / `pptx_inspect_format` / `pptx_list_files` 照常可见）。清单与绊线在 `ToolDeclarationContractTest`。**工具名前缀决定宿主可见性**：Excel 面必须 office_excel_*、PPT 面必须 office_ppt_*，其余 office_* 归 Word——起错前缀会漏进错误宿主的会话（死路径）。
...
- **`OfficeBridgeService` 的超时按 command 分级**（`ACTION_TIMEOUT_SECONDS`，13 条 120s：Word 面 `replace_batch`/`apply_standard_format`/`insert_table`/`set_numbering`/`format_table`/`insert_image`，Excel 面 `excel_set_values`/`excel_set_formulas`/`excel_add_pivot_table`，PPT 面 `ppt_add_slide`/`ppt_add_table`/`ppt_replace_text`/`ppt_format_text`；其余 30s）。这与 `EditorBridgeService` 是同一条纪律，理由也一样：**平超时是「后端先放弃、模型重发一次造成双改」的成因**。凡是「一次调用做 N 件事」的原语都必须进这张表——忘了加的症状不是报错，而是内容被写两遍，最难查。**现在有护栏**（dev-board#806）：`OfficeBridgeTimeoutCoverageTest` 扫 OfficeEditTools 源码，凡带 `...Json` 批量参数的工具不在表里即转红（自动发现，新增即受管）；参数看不出批量、实现却要遍历整篇的（apply_standard_format / set_numbering / format_table / insert_image / ppt_add_slide / ppt_replace_text / ppt_format_text）在那条用例的「判断档」里逐条写了理由。**读取类刻意不进这张表**：LOWA 桥把读取类抬到 120s 是 dev-board#729 ③ 拿真机 telemetry 换来的，插件桥没有同等证据，而读取超时只白等一轮、不会双写——别跟着抄。
...
- get_text 分页（dev-board#806）：`node --test office-addin/taskpane/lib/textPaging.test.js office-addin/taskpane/lib/officeGetTextPaging.test.js`（切法 + 两个家族的接线）。
- Word 面补齐的三条命令：`node --test office-addin/taskpane/lib/officeWordFaceGaps.test.js`（Office 面）与 `wpsWordHandlers.test.js` 末尾那组（WPS 面）。
- 两族对拍（含 skill 白名单）：`node scripts/check-tool-parity.mjs`（已进 CI 的 spdx-check job）。
...
- **大区域必须先 `Resize` 再取 `Value2`**：`excel_get_range` 只回 500 行、`readEtSheet` 只附 2000 行，却都把整片已用区域编组过同步桥，几万行的台账「一问就把 WPS 卡死几十秒」。截断要发生在过桥之前；`excel_search` 另加 `MAX_SEARCH_SCAN_ROWS=5000` 扫描上限并在返回值里如实交代扫到哪儿（不假装搜完了）；`excel_replace`（dev-board#804）同一口径——值与公式各一次批量读回，**只有真正要改的格才逐格赋值**（过桥次数 = 改动格数，不是区域格数）。
- **`excel_replace` 只重写命中的格，公式格与数值格一律不动**（dev-board#804 / 审查 A16·B-04）：Excel 两族此前都只有只读的 search，成批改写只能退回 `excel_set_values`——那是按矩形区域写的，散点命中整块回写会把区域内不该动的格子一起覆盖掉。判定层（`excelReplace.js` 的 `planCellReplacement` / `normalizeReplaceArgs` / `buildReplaceResult`）**Office 与 WPS 共用同一份**，与 `textMatch.js` / `minimalEdit.js` 同一类角色：同一条指令在 Excel 和 WPS 表格上给出不同结果，比两边都做不到更糟。刻意不用 `Range.replaceAll`——它按区域整体处理，公式格与数值格都会被卷进去，也拿不到「改了哪几格」的账。匹配是**字面量不是正则**（「（甲方）」「*备注」带正则元字符）。省略 `replace` 报错，不当成「替换成空串」。单次上限 `MAX_EXCEL_REPLACEMENTS = 2000`，与后端 `OfficeEditTools.MAX_EXCEL_REPLACEMENTS`、桌面端 `DocumentEditTools.MAX_SHEET_REPLACEMENTS` 同值。已进 `EXCEL_UNDOABLE_COMMANDS` / `ET_UNDOABLE_COMMANDS`（批量替换必须能在修订记录里回退；`rangeAddress` 留空时快照跟着走已用区域，否则最常用的「全表替换」就没有撤销点）。回归 `office-addin/taskpane/lib/excelReplace.test.js`（16 条，两族各盯「未命中的格一次都不写」）。
- **`excel_set_values` 默认沿用上一行格式**（dev-board#844，病灶：在「重大合同清单」末尾加一行，新行无边框、字体不同、对齐不同——写值原语只写值，读取原语只回值，模型既没被要求对齐也看不到格式）。判定（`excelFormat.planFormatInheritance`，两族共用）四条缺一不可：① `inheritFormat` 没显式传 false；② 该行写入前在写入列跨度内全空（**非空行一律不动格式**）；③ 本次写的值不全空；④ 紧邻上一行本身是本次沿用了格式的行（**级联**），或者它与再上一行**写入前都有内容、且格式签名在写入列中至少一半的列一致**（`rowsFormatConsistent`：签名 = 字体名/字号/加粗/水平对齐/数字格式/四边有无边框，**刻意不含字色和底色**——状态列标红、隔行底纹在数据行里很常见）。只有一行时那多半是表头；两行都有内容但格式对不上多半是「标题行（合并居中）紧贴表头」——两种情形把表头的加粗/底色扩给第一条数据恰恰是错的，与桌面端同一规则（桌面端真机用例「标题紧贴表头：无 formatInherited」）。表头下本次新写的第一行也不能反过来给下一行当来源；级联分支不再比格式。**只在有候选行时才读格式**（`formatCheckRows` 为空就一格不读，改既有数据、写在空白处不多一次跨桥）；Office ≥1.9 用 `getCellProperties`、以下逐格 load，WPS 逐属性读；`rowFormats` 缺失按不一致处理（宁可不沿用）。级联的多行一律从链条最上端那行既有数据（`root`）复制，返回值 `formatInherited: [{row, from}]` 报的是紧邻上一行。**尽力而为**：沿用失败只在返回值 `formatNote` 里交代，值照写。落笔：
  - Office：ExcelApi 1.9 用 `Range.copyFrom(源行, RangeCopyType.formats)`（Learn 核实 1.9，不经系统剪贴板）；**1.9 以下不跳过**，逐格读字体/填充/对齐/换行/数字格式/四边边框再写（律所的 2016/2019 永久版不少，跳过等于这批用户的病照旧）；无填充在 1.9 以下读回是 `#FFFFFF`，按无填充 `fill.clear()` 而不是涂白（白底盖网格线）。行高另抄（ExcelApi 1.2），**源行开了自动换行时不抄**——固定行高会截掉新行里更长的文字。
  - WPS：逐属性复制（Font 六项、Interior、对齐、NumberFormat、WrapText、四边 Borders、RowHeight），每项各自 try、失败计数进 `formatNote`。**不许用 Copy + PasteSpecial**（改写用户剪贴板）；`Range.Copy(Destination)` 虽不经剪贴板但会连值/批注/数据验证一起搬，且 WPS 上没实测，也没用。
  - **两族都「只画有线的边、不写 None/xlNone」**：相邻格共用一条边，给新行上边写 None 会把上一行的下边框一起擦掉。
  - 已知局限：修订记录/跨文档撤销的快照只含值（`captureExcelState` / `captureEtState` 取的是 formulas），撤销一次追加会把值清回空、沿用来的格式留在那一行。
- **`excel_get_range(withFormat=true)` 的格式摘要**：只扫前 `MAX_FORMAT_CELLS = 400` 格（`formatScanShape` 先保列再截行，超了标 `format.truncated`）；`baseFont` = 区域内最常见的字体名/字号，各格只在与它不同时才报 fontName/fontSize；每列取**出现至少两次的最常见差异**当列级摘要，与之不同的格（典型是表头）进 `cells`，值是该格完整的差异项（`{}` 表示这格是默认外观）。「默认」口径：不加粗/不斜体/黑字/无填充（白填充也算无）/水平 general/垂直 bottom/常规数字格式（含 WPS 中文界面的 `G/通用格式`）/无边框/不换行。边框概括成 `all` 或 `top,bottom` 这类边列表，非细线追加 `(medium)`。Office 1.9 走 `getCellProperties` 一次取回，以下逐格 load；WPS 逐格读（400 格约 7000 次跨桥调用，约 1-2 秒，是可选参数所以可接受）。
- 回归：`node --test office-addin/taskpane/lib/excelFormat.test.js office-addin/taskpane/lib/excelFormatHosts.test.js`（纯函数 20 条 + 两宿主 mock 17 条；WPS mock 的 Copy/PasteSpecial 一调就抛；「标题紧贴表头 → 不沿用」与「只差字色/底色 → 仍沿用」两族各有一条）。
