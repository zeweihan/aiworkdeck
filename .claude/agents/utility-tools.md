...
**文件缓存区（左下角「文件暂存区」）**：`FileStagingArea.vue`（纯展示，用量条读 `usage` prop）+ `pages/project-overview/stagingArea.js`（方法组）。`FileStagingArea.onDrop` 第 3 分支（真实 OS 文件 → `drop-files` → 宿主 `onStagingDropFiles` 借 `FileTree.confirmUpload` 上传）读 `dataTransfer` 必须回落到 `window.event`——uni 重建的 `<view>` 事件没有它，详见 sidebar-shell.md「drag 系事件是重灾区」。**物理形态是项目内名为 `__staging_area__` 的文件夹**，「加入缓存区」= `batchMoveFiles` 把已有项目文件移进去，没有独立的缓存区目录。
