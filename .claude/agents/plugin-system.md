...
**产品口径：中文姓名不自动识别（2026-09-09 维护者拍板，dev-board#531）。** `SensitiveType.CHINESE_NAME` 的模式是 `[一-龥]{2,4}`，中文文书里几乎每个词都是 2~4 个汉字，「甲方」「北京市」「有限公司」「被告」统统命中，整篇被涂成 `甲*／被*／有**司`；而中文姓名没有身份证的 mod-11-2、银行卡的 Luhn 那样的客观校验位（#521 那套 `isPlausible` 收紧法在这里用不上），纯正则分不出「张三」和「本条」。理由一句话：**法律文书里漏涂比误涂安全**——文书必须逐字可引，把正文改坏的代价比漏一个名字更大，漏涂还有人工复核兜底。落地形态：
- 枚举值 `CHINESE_NAME` **保留**（存量记录里可能存着这个 code、老客户端可能还会传），但带 `autoDetect=false`；`SensitiveType.autoDetectTypes()` 是唯一的「可自动检测类型」来源，`GET /options` 只列它，`SensitiveService.replaceSensitiveData` 与 PDF 侧 `finishPage()` 各有一道 `isAutoDetect()` 闸——硬传进来也只记一行日志、不产生任何改动。加新类型时默认 `autoDetect=true`（5 参构造器），要下线才显式传 `false`。
