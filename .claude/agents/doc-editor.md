...
- **外部改名 / 移动（BUG-14，v0.49.0 真机 C4-03）**：旧对账把 Finder 里的改名拆成「旧行进回收站 + 新名一条新行」，编辑器手里的旧 fileId 照旧保存，`save()` 的 createDirectories + REPLACE_EXISTING 在旧路径把旧文件名重新建出来——磁盘两份、三处名字对不上。现在三段：①`LocalProjectService.reconcileProject` 先收集「库里活着、磁盘上已不在」的文件行，导入时遇到没有对应行的新文件，按**大小相同 + 扩展名相同 +（同目录 或 同名）且恰好一个候选**认领，经 `ProjectFileService.relinkExternallyMoved` 原地改 name/parentId/filePath（**id 不变、不碰磁盘**；认不准就退回旧语义）；②编辑器保存带 `?mustExist=1`（条件：`file.fileSize > 0` 或本会话已存过一次——新建空白文档的第一笔不带），后端对「行已进回收站 / 目标路径不存在」回 **409 `reason:FILE_MOVED`**，绝不在旧路径重建；前端落成 `statusKey='movedSaveFailed'`（「文件已被移动或改名，改动尚未保存」），留脏、给重试；③工作台订阅 `awd:files-changed`（文件树每次重载的汇聚点）→ `syncOpenTabsFromFileTree()` 按 id 对齐标签/状态栏/窗口标题。不带 `mustExist` 的上传（新建文件先建行再传字节、录音分片）行为不变。测试：backend `LocalProjectServiceTest`（external* / ambiguous* / unrelated*）、`FileControllerMovedFileGuardTest`；frontend `tests/project-home/external-rename-save.test.mjs`。**未做**：moved 出项目（行进回收站）时的「另存到别处」选择；不存 inode，改名同时改了大小（外部编辑器改完再改名）认不出。
