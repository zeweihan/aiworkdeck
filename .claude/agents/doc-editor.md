...
- **低内存引擎预算（dev-board#721）**：上述体积权重之外，宿主 `host.systemMemory.totalBytes` ≤ 8GiB 或缺失/无效时仅在没有常规文档引擎时预热首套空白引擎，已有文档（包括尚未 mounted）或备胎已过继后不再补第二套空白；全部文档关闭后延迟 4 秒恢复首开预热，期间重开文档则取消补胎。常规驻留目标限制为 2 个引擎（单窗格 = 当前文档 + 最近后台文档；双窗格的活动文档优先占名额）。已知 >8GiB 设备保留原策略。读取的是 Electron 主进程传给 preload 的物理内存快照，不用浏览器 `navigator.deviceMemory` 的截断值。实例上限与体积预算任一超限即沿既有 `flushSave` 淘汰链处理；保存失败必须留住，保存期间重新激活/回到预算内也不淘汰。三方合并的 hidden 实例仍按需创建/释放，不属于后台预热。**不要恢复 Chromium 后台节流或 display:none**，会重新触发 #539 的冻结问题。测试：`frontend/tests/project-home/libre-pool-memory-policy.test.mjs`。

## 自动补全（本地写作辅助，dev-board#538；#755 改名并移入工具栏）

- 对外叫**「自动补全」**（en: Autocomplete；#755 之前叫「写作辅助」，代码里的 `attachWritingAssistance` / `.awd-writing-assistance` / `writingAssistance*` 等内部名一律不动）。面板内那个开关叫**「自动弹出候选」**（en: Suggest as I type），别跟面板同名。
- **设置面板的唯一入口在 Writer 自建工具栏**（#755）：画布右下角那颗常驻按钮 `.awd-wa-toggle` 已撤掉（连同它的 CSS），`EditorToolbar.vue` 右侧常驻区里「自动补全」与「有据续写」由一条 `etb-sep` 起头并排成写作一组（两颗都不可用时那条分隔线也不画）。消息对与 #748 同款、宿主不本地乐观翻按下态：客体 → 宿主 `writing-assistance-state {available, open}`（`available = session && writable`；`open` 只算设置面板这一族 `mode ∈ {settings, manage}`——打字候选、右键查询卡片、资料卡都不点亮那颗按钮），宿主 → 客体 `writing-assistance-panel {open}`。`LibreOfficeEditor.initWritingAssistance()` 换文档时先把 `writingAssistanceAvailable/Open` 复位，等客体重新上报。面板关着时客体根节点整块 `hidden`（画布上不留一个像素、也截不到点击），根节点另挂 `data-available="true|false"` 作为「这份客体的写作会话已就绪」的 DOM 标记——`tests/desktop-e2e/writing.mjs` 认 webview 靠它（原先靠那颗按钮的 `hidden`）。
- 右键查询（机构/法规/案例）**不受「自动弹出候选」开关影响**，默认开、点了才联网；设置面板底部用一行灰字说明这件事（`LABELS.contextHint`），不在那里再给一个开关。
...
- 文档候选按同一 revision 分页读取（最多 1,000 段/200,000 字/500 候选），首次读取与显式刷新才扫描；首次读取被编辑打断时，等待 1.2 秒停笔后重试，成功后不随每次按键重扫；机构、人名、法规、表述按类别保留名额。完整法规名称用于展示，裸法规/条款前缀使用匹配的补入形式；不得补出半个书名号。持股叙述的人名、机构前的“股东名册中”等已覆盖回归。**分页正文读取必须带 `__agent: true`**（`writingAssistanceHost.seedDocument`，dev-board#725）：它是「要最终文本」的唯一表达（经 worker 的 `runAgentCommandInMarginView`，无修订文档上是零开销）；内联「全部修订」是产品默认视图，不带这个标记读回来的正文混着被删的旧字，抽出来的词库是错的。**这里原先是「`set_revision_view` 探一下，`mode === 'all'` 就直接 return」**——默认从页边改成内联之后，等于在每个用户都在的视图里把文档词库整个关掉了（`test:writing-caret` 因此在 master 上一直红）。别再按显示方式给采集设闸；单测 `tests/completion/host.test.mjs` 的「the inline all-markup default still seeds document vocabulary」钉住这条。
...
- UNO 三动作 `get_completion_context` / `accept_completion` / `insert_completion_content` 以不透明 token 校验模型、光标/选区两端及上下文；补全只追加后缀，表格/纯文本原样插入，整组一次撤销。真实修改使 token 失效，只读导出期间保留 snapshot（含恢复 modified 标志），不能因自动保存误拒插入，也不能放宽位置校验。行内“全部修订”视图经 `FINAL_TEXT_ACTIONS` 临时以最终文本语义取值和落字，再恢复原视图。
- **语义写作的两条独立原语**（dev-board#709）：`capture_writing_context({radius?, composing?})` 返回顶层 `token/revision/before/after/paragraph/selectedText/hasSelection/writable`；`revision` 为数值，选区不超过 2000 UTF-16 单位时也可预览（`available:true`）。`accept_writing_suggestion({token,text,expectedSelection?,revision?,composing?})` 的 `text` 是已经去掉前文的**插入后缀**，有选区时则是该选区的完整替换内容；非空选区必须传逐字相同的 `expectedSelection`。不接受跨位置替换、Markdown 转换或超过 2000 UTF-16 单位的内容，允许换行并落成真实段落。接受在同一条同步 worker 命令里检查 model、全局 revision、游标两端、原文与选区；单次采用合为一次撤销，失败回滚复用 `completionEdit`。两动作加入 `FINAL_TEXT_ACTIONS`，在行内全部修订态下临时按最终文本执行后恢复视图。成功回 `inserted/replaced/operation/revision`；`operation` 为 `insert-suffix` 或 `replace-selection`。
- **语义快照与即时词条快照分开**：`writingSnapshot` 保留生成请求的上下文，普通 `get_completion_context` 刷新不会覆盖它；全局 revision 仍使其他段落的修改也令它过期，只有只读导出保留。`readonly`（含比较只读模型）、`stale`、`selection-mismatch`、`invalid-suggestion` 等错误不得降级为不校验的普通插入。worker 无法观察 DOM 输入法状态，`composing:true` 仅是额外拒绝；guest 必须在真实 composition 期间阻止请求和采用，不能只依赖调用者参数。生成文本修订需要 AI 署名时沿用 `__agent:true`，不另造署名通道。
- **当前作用域必须来自真实光标**：`capture_writing_context` 另回 `paragraphIndex/sectionPath/sectionTitle/scopeKnown`。`writingSectionOf` 从 live range 定位正文段落，并识别 OutlineLevel 或普通文字“附件一／第X条”；附件命名空间在其内条款编号后仍保留，如 `附件一 / 第一条`。无标题但确认在正文时回 `正文`，无法定位的 story 拒绝为 `unknown-writing-scope`，跨章节选区拒绝为 `selection-crosses-writing-scope`；不能把未知作用域默认为文首。宿主正文读取必须带 `__agent:true`，与 capture 同用最终文本语义，避免删除修订混入事实；当前段落未完整进入所截正文范围时，不能声称该局部定义已解析。
...
- 回归：`npm run test:completion`（含 `tests/completion/autocomplete-toolbar.test.mjs` 宿主接线 + `controller.test.mjs` 里客体的开合/回报/命名三条）、`test:lowa-completion`、`test:writing-ui`、`test:writing-caret`、`test:lowa-link-preview`、`test:writing-desktop`；引擎测试包含移动/输入/重载拒旧 token、跨导出仍可插入、撤销/重做及资料表格。桌面用例从真实项目词库经中文输入/Tab 到自动保存后下载 DOCX 核对，夹具文件隔离在临时目录。
- 语义原语回归：先 `npm run build:zetaoffice`，再 `LOWA_ENGINE_DIR=<已准备的引擎目录> npm run test:semantic-writing`（真实引擎 `tests/lowa-e2e/semantic-writing.mjs`）；验证多段原样后缀、精确选区、一次撤销/重做、普通补全刷新、其他段落修改、换文档、自动保存、长度和控制字符、显式 composition 参数、比较只读模型。此专项不代表已经验证表格/脚注全部位置，也不替代 guest 的真实 IME 与菜单点击验收；菜单/Tab 接线仍跑 `test:writing-ui` 与 `test:completion`。需要并行运行引擎服务时设置不同 `LOWA_E2E_PORT`，不同时覆盖 `dist/zetaoffice`。
...
- **外部改名 / 移动（BUG-14，v0.49.0 真机 C4-03）**：旧对账把 Finder 里的改名拆成「旧行进回收站 + 新名一条新行」，编辑器手里的旧 fileId 照旧保存，`save()` 的 createDirectories + REPLACE_EXISTING 在旧路径把旧文件名重新建出来——磁盘两份、三处名字对不上。现在三段：①`LocalProjectService.reconcileProject` 先收集「库里活着、磁盘上已不在」的文件行，导入时遇到没有对应行的新文件，按**大小相同 + 扩展名相同 +（同目录 或 同名）且恰好一个候选**认领，经 `ProjectFileService.relinkExternallyMoved` 原地改 name/parentId/filePath（**id 不变、不碰磁盘**；认不准就退回「旧行出索引 + 新建一行」）；②编辑器保存带 `?mustExist=1`（条件：`file.fileSize > 0` 或本会话已存过一次——新建空白文档的第一笔不带），后端对「行已进回收站 / 目标路径不存在」回 **409 `reason:FILE_MOVED`**，绝不在旧路径重建；前端落成 `statusKey='movedSaveFailed'`（「文件已被移动或改名，改动尚未保存」），留脏、给「重试保存」与「另存为…」（`saveCopyAs`：导出当前内容走 blob + a[download]，桌面主进程接管成系统另存为对话框，绝不上传、改动仍算未保存）；**404 也落到这个状态**——dev-board#903 之后对账认不出的外部删除 / 移出项目直接把行出索引，编辑器按旧 id 保存在 mustExist 围栏之前就撞 404；③工作台订阅 `awd:files-changed`（文件树每次重载的汇聚点）→ `syncOpenTabsFromFileTree()` 按 id 对齐标签/状态栏/窗口标题，编辑器 watch `file.name`：处于 movedSaveFailed 时名字一变就自动 `retrySave()`（对账认领成功 = 行已改指新路径）。不带 `mustExist` 的上传（新建文件先建行再传字节、录音分片）行为不变。测试：backend `LocalProjectServiceTest`（external* / ambiguous* / unrelated*）、`FileControllerMovedFileGuardTest`；frontend `tests/project-home/external-rename-save.test.mjs`。**未做**：不存 inode，改名同时改了大小（外部编辑器改完再改名）认不出。
...
- **往视图光标写字必须用 `vc.getText()`，不是 `xModel.getText()`**（dev-board#627，真机实证）：正文 XText 只接受属于自己的区间，光标在**表格单元格**（或页眉/脚注等别的 story）里时 `xModel.getText().insertString(vc,…)` 抛 `RuntimeException`——IME 提交（`insert_at_cursor`）、粘贴（`replace_selection`）、回车（`insert_paragraph`）整条静默失败，用户看到的就是「表格里打不进字」。与删除路径、修订显示方式都无关（不删直接打也一样失败），删除只是撞上它的场景：页边/气泡视图把删掉的字藏起来，单元格于是看着全空。`insertInlineStyled` 一直用 `vc.getText()`，所以带 markdown 标记的插入在单元格里反而是好的——新增写光标的原语照它的写法。**同族已修**：`anchorBookmark()` 也曾用 `xModel.getText().insertTextContent` 给任意 range 插锚点书签，单元格区间上同样抛 RuntimeException（异常被 `find_text_locations` 的 catch 吞掉 → `anchorId: null`），AI 的 `set_selection` / `replace_at_position` 于是在表格里一律回「anchor not found」；已改成 `range.getText().insertTextContent(...)`（`bookmark_selection` 本来就是这么写的，从未受影响）。**判据是「往谁的 range 写」**：`xModel.getText()` 用来枚举正文（`buildParaIndex` / `get_outline` / `applyHouseToDocument`）或当 `XTextRangeCompare` 比较基准（`paraKeyOf` / `headingChainOf`，跨 story 抛异常是已接受的 -1 语义）都是合法的；`removeTextContent` 本引擎实测容得下别的 story 的书签（`clear_anchors` 用正文 XText 照样摘得掉单元格锚点，`table-retype.mjs` 锚点组守着），只有**插入/写入**必须换成区间自己的 XText。
- **Tab 在表格里是跳格，不是制表符**（#627 的次生问题）：覆盖层此前无条件 `insert_at_cursor('\t')`，在单元格里正因上一条抛异常而静默失败——「Tab 没反应」把它一直掩护着；`vc.getText()` 修完之后同一条分支会真把制表符插进单元格（修订态下还多记一条修订）。现在覆盖层只转发一条 `tab_key {shift}`（已入 `EDITOR_ACTIONS`，宿主发起非 AI 管线），**判定放在 worker**——光标属于哪个 story 只有引擎知道：`vc.getPropertyValue('Cell')` 非空就派发 `.uno:JumpToNextCell` / `.uno:JumpToPrevCell`（2026-09-14 真机实证，24.2.8-zhcn-r5 上两个槽都生效，A1→B1→A2 与反向逐格对得上，**不是哑弹**；末格 Tab 引擎自己新增一行，与 Writer 一致，白拿的），否则走原来的 `insertTextAtCursor(vc, '\t')`。正文里 Tab 与 Shift+Tab 都仍插制表符（改造前就是这样，别顺手把 Shift+Tab 改成别的）。补全菜单打开时 `onAssistanceKey` 先接管 Tab，这条在它之后。
...
- **真正会随上下文冒出来的是 `*objectbar` 那一族，不是 `singlemode-*`（v0.44.1 真机走查 D2，2026-09-15 真机 r5 实证）**：boot 时 `getElements()` 里 11 条 `singlemode-*` 早已创建且不可见（所以 `hideElement` 对它们有效，e2e 一直绿），而光标一进表格单元格，引擎当场创建并显示 `private:resource/toolbar/tableobjectbar`——这个名字过去根本不在 `CHROME_URLS` 里，谁都没藏过它；「定位」类跳转拉起同族的 `navigationobjectbar`（箭头 + 对象类型列表那条）。现在 `CHROME_URLS.toolbars` 收了 12 条经典上下文工具栏（tableobjectbar / navigationobjectbar / graphicobjectbar / frameobjectbar / oleobjectbar / drawingobjectbar / drawtextobjectbar / bezierobjectbar / numobjectbar / previewobjectbar / fullscreenbar / changes）。
- **`hideElement()` 对一条「还没创建」的元素不留痕**（同上实证）：预先 `hideElement('…/tableobjectbar')` 之后光标第一次进表格，`isElementVisible` 照样回 true。所以 `set_chrome` 的藏那一侧现在是 **`createElement` 再 `hideElement`**——先把元素造出来再藏，引擎就记住了「这条是藏着的」，之后进出表格都不再冒出来（13 条一起 create+hide 实测 115ms，只在开关时跑一次）。同轮否掉的两条路：往 `/org.openoffice.Office.UI.WriterWindowState/UIElements/States` 写 `Visible=false` + `ContextSensitive=false`（写得进、读得回，**不生效**）；`XLayoutManagerListener` 装得上但收不到 `UIELEMENT_VISIBLE` 事件。
- **原生菜单模式（逃生开关）的两处裁剪（BUG-33，v0.49.0 真机 C4 观察 3/8）**：①`CHROME_NEVER_SHOWN`（目前只有 `fullscreenbar`）在 `set_chrome` 的显示侧也按「藏」处理——顶层窗口被 `ensureFullScreen()` 钉成全屏，那条只有一个「全屏」按钮的浮条压在样式框上，点它会把画布退回带标题栏的小窗；②`NATIVE_MENU_BLOCKED`（`.uno:Quit` / `.uno:OpenRemote` / `.uno:WebHtml`，即「退出 ZetaOffice」「打开远程文档」「在浏览器中预览」）两道闸：放出菜单栏时 `pruneNativeMenu(lm)` 改本 frame 菜单栏元素的 `getSettings(true)`/`setSettings`（递归删条目 + 收拾悬空分隔线，结果回在 `applied.menuPruned`），派发层 `installReviewCommentInterceptor` 对这三条 `queryDispatch` 回 null（Ctrl+Q 失效，菜单没裁掉时也只是灰掉）。单测 `frontend/tests/lowa-unit/nativeMenuPrune.test.mjs` 用假 LayoutManager/假菜单容器跑真实现；**真引擎上 XUIElementSettings 改菜单是否生效尚未实测**（`applied.menuPruned` 为 0 即说明没接上，派发层那道闸仍在）。
- 宿主侧的可重入入口是 `EditorToolbar.vue` 的 **`reapplyChrome()`**（按当前开关状态重跑 `applyChrome` → `set_chrome`，没接执行器就不做）：`load_document` 会把藏好的 chrome 重新拉出来，换文档 / 重载之后调它一次即可，上下文工具栏走同一条路，不需要另外的上下文钩子。
...
- **产品默认现在是 `DEFAULT_REVISION_VIEW = 'all'`（内联全部修订，office_thread.js:2120），与 Word 一致**。曾经默认是 `'margin'`（页边），理由是 AI 文本读取契约：页边把删除的文字移出正文流，于是 `get_document_text` / `get_paragraph` 读到的正文就是「改后的样子」，`find_text_locations` / `replace_nth_match` 的 matchIndex **只数可见匹配**（dev-board#369 的用例明写了这条）。**那条契约一个字都没变，只是改由 `runAgentCommandInMarginView` 兜住**——AI 命令（`__agent`）与写作/审校的只读取文命令（`FINAL_TEXT_ACTIONS`）在内联视图下临时切成「最终文本语义」再切回来。所以：内联是**显示**默认，最终文本是**读取**语义，两者分离；写新的读取原语时要么进 `FINAL_TEXT_ACTIONS`、要么自己带 `__agent`，否则读到的正文会混进被删的旧字。
- **AI 命令另有守卫，让用户可以放心切内联**（`runAgentCommandInMarginView`，execCommand 里对带 `__agent` 的命令与 `FINAL_TEXT_ACTIONS` 生效）：当前视图是 `all` 时，执行前临时切到一种不含删除文字的态 + `refresh()`，执行完切回用户所选的态 + `refresh()`；页边态与最终稿态的正文本来就不含删除文字，直接放行零开销（默认路径因此没有任何额外成本）。豁免 `set_revision_view` / `export_document` / `load_document`（前者会打架，后两者自带包装）。非 Writer 一个属性都不碰。原语可能是 async（分批的 `find_replace` / `apply_house_style`），恢复必须等它 settle，同步抛异常也要还原。**还原病灶即转红**：摘掉守卫后，内联态下 `get_document_text` 读回 `甲方一、甲方二、甲方三。`（混着被删的字）、`find_text_locations` 数出 3 处而不是 2 处（e2e 组 32 的守卫段 3 条转红），装回即绿。
...
- **页外审阅区宽度 `AwdReviewSidebarWidth` 只有两处可以写**：用户经 `set_revision_view` 从别的模式切进 balloons（`applyRevisionView(mode, reserveGutter)` 预留 280）与宿主的 `set_review_balloons`（仅插入/无批注时释放为 0）。导出、修订处置、AI 读正文这些临时切视图再切回的路径**一律不写宽度**（临时视图本来就不改它），否则每次自动保存都把宿主已释放的空栏重新撑开：整页重排、横向跳两次。单测 `tests/revision-view/review-gutter-restore.test.mjs`。**这条宽度是 0/280 的全有全无量，每换一次整页横向重排一次**，所以它的判据必须只认「这个视图真会画出来的卡片」，且要对称迟滞：`reviewLayout` 的 `add()` 里 `pending`（未排版卡片）只在 `revisionCards`（balloons/margin）时才计修订——内联视图下 `data` 只有 `{index}`、没有 `type`，`data.type !== 'Insert'` 恒真，于是每条还没排版的修订都占住 280，Writer 空闲排完后又释放成 0（300 段夹具实测 pending 100 → 44 → 0，用户看到的就是正文和光标左右跳，dev-board#725）；宿主 `zetaOfficeReviewBalloons.refresh()` 侧则要**连续两次读取结论一致**才动宽度（`lastWantGutter`，不一致就 250ms 后再看一次，别用 `schedule(0)` 去压单线程的 office 循环），`unread` 两个方向都计（原来只在 `enabled` 时计，是不对称的）。回归 `npm run test:lowa-review-gutter`（`tests/lowa-e2e/review-gutter.mjs`，边打字 5 秒断言宽度取值集合大小为 1）。
...
- **打字路径不许等审阅元数据**（dev-board#587/#600，2026-09-11 真机实测）：本引擎上一次 UNO 调用若返回对象（枚举、区间、文本光标）要 3–7ms，`list_revisions` 每条约 55ms（段落定位器 27ms，且 modified 后重建整份段落索引），8 条修订一次读就要 0.5–1.2s。气泡层早先每次编辑 80ms 后就整份重读，中文确认后的文字要等约 1s 才画出来——这就是「选中候选后要点空白才显示」在新引擎上的根因。现行契约：`get_review_layout({fresh:false})` 只读原生几何（约 20–40ms），元数据按原生身份缓存复用（修订用 `AwdReviewGeometry` 的 `id`=SwRangeRedline::GetId，不是 UNO 的 `RedlineIdentifier`；批注用 id），位置随几何实时更新；`stale`/`unread` 为真时气泡层在停笔 900ms 后再 `fresh:true` 重读，卡片操作后立即重读。布局读用 `list_revisions/list_comments({locate:false})` 跳过段落文字与定位器；非页边/气泡视图根本不读修订元数据。卡片键与签名只取渲染与围栏字段，编辑别处不重建卡片。`pending`＝已知卡片所在页 Writer 尚未排版（空闲排版不发事件，气泡层 700ms 轮询至多 6 次），`unread`＝原生有而缓存未读。宿主审阅面板的 modified 刷新也改成停笔 1s 后。回归：`ime-commit.mjs`（**必须有头浏览器跑**：无头 Chrome 只有截图触发的 resize 才呈现引擎画布，而 resize 本身会让 Writer 重画，分不清自画与被戳）、`review-performance.mjs`、单测 `native-review-geometry.test.mjs`。
- **页边/气泡视图下的撤销只能在记录它的那种视图里做**（2026-09-11 r4、r5 均复现）：ShowChangesInMargin 开着时，撤销一条被拒绝的长删除会让引擎永久卡死（`get_selection` 两分钟后仍超时，用户未保存内容全丢）；而把页边下录的编辑拿到内联视图去撤销同样卡死（lowa-e2e 撤销/重做组实证）。现行规则：页边/气泡视图下的接受/拒绝在 `runRevisionResolution` 里临时切内联执行，并按标题+栈深记住这条撤销项；`undo/redo`（含拦截的 Writer 自身 `.uno:Undo/.uno:Redo`）只在栈顶正是这类记录项时才同样绕道内联（`undoStep`），其余一律原地撤销。别再写「所有撤销都切内联」——那正是把组 4 打挂的版本。
- **`mega-build.sh` 自己写 autogen.input**，必须带 `--with-wasm-module=calc writer impress`（与 `autogen.input` 一致）：2026-09-11 第一版 r5 产物漏了这行，soffice.data 只有 1656 个文件（r4 为 1935），`simpress/sdraw/smath` 全缺，PPTX 编辑与 `slide_*` 原语会全挂。验收新引擎先比 metadata 文件数与 `simpress` 条目。
...
- **临时切视图取最终文本，只能走「隐藏修订」这条路，不能走「页边」**（dev-board#604，2026-09-13 真机 r5 实测）：`ShowChangesInMargin` 是**控制器的视图设置**，每写一次引擎就把视图滚回光标；`RedlineDisplayType`（隐藏修订）是**模型属性**，一个字节都不碰视图。默认视图在 #812 改成内联 `all` 之后，`runAgentCommandInMarginView` 成了常态路径，于是用户往下滚动、180ms 后即时审校打一条 `get_review_context`，视口就被拽回光标所在的文首——用户报的「文档往下滚动时自动跳回开头」。实测数字：切页边 `ctrl.getViewData()` 的 VisibleTop 44880 → 0；切最终稿 21840 → 21840 纹丝不动；两者读回的正文 / 段号（`paraKey`）/ 偏移 / `list_revisions` 坐标一字不差，整段被删的段落在两种态下同样不进段落枚举。所以守卫里是 `if (applyRevisionView('final').mode === 'all') applyRevisionView('margin');`——先隐藏，引擎隐不掉（`hideSupported` 为假）才退回页边。**别把它改回单调 `applyRevisionView('margin')`**。其它可能的修法真机全否：`ctrl.restoreViewData()` 在本引擎是 `SfxBaseController` 的空实现（写进去读回来纹丝不动，不报错）；`xModel.lockControllers()` 只把滚动推迟到 `unlockControllers()` 那一刻照跳；`xModel.refresh()` 不是触发源（不 refresh 照跳）。跳的落点是**视图光标**不是文首（光标在第 120 段时视口跳到第 120 段），光标本来就在视口里时不跳。回归 `npm run test:lowa-scroll`。
- **审阅区宽度 `AwdReviewSidebarWidth` 的 280↔0 抖动不会造成纵向回顶**（同轮实测排除）：视口 top 19920 时来回写 280 / 0，VisibleTop 一动不动。
- **内联态下读不出「结果文本」**：正文里旧字新字同框（把「三十」改成「六十」，`get_document_text` 读回「六三十」；表格单元格同理）。内联现在是默认态，**凡是「改完读回来核对」的逻辑都要留意这条**——AI 侧与写作/审校侧由 `runAgentCommandInMarginView` 兜住，宿主侧自己发的**不带 `__agent`、也不在 `FINAL_TEXT_ACTIONS` 里**的读取命令（工具栏、审阅面板、`writingAssistanceHost.seedDocument` 的 `get_document_text` 分页读）看到的是原始内联正文。lowa-e2e 里切模式的断言集中在组 31/32。
- **守卫的三条纪律（dev-board#725，真机实证 24.2.8-zhcn-r5）**，`runAgentCommandInMarginView`：①**一条修订都没有就整段跳过**（`hasAnyRedline()`，一次 O(1) 枚举探问，不取元素——取一个元素就是 3-7ms）。这趟往返不便宜：两次模型属性写 + 两次 `invalidateParaIndex()`（下一次读要把整份段落索引重新枚举）。`completionGuard()` 必须跟着放行（`mode === 'all' && hasAnyRedline()` 才拒），否则起草场景下补全与即时审校一律回 `inline-revisions`——这是改造时自己踩进去的坑。②**守卫里不许 `xModel.refresh()`**：实测隐藏修订后不重排，`getString` / 段落枚举 / 区间偏移读回的最终文本与「切+refresh」逐字相等，而 refresh 是整份文档重排，这条守卫在默认内联视图下是 35ms/180ms 节拍的常态路径。**导出那条（`withInlineMarkupForExport`）仍然必须 refresh**，那里消费的是版面不是文本，别把两处当同一回事。③**还原视图后必须补一次正文读**（`paragraphTextOf(ctrl.getViewCursor())`）：Writer 把光标的**屏幕**坐标缓存着，只在下一次读正文时按当时的视图几何重算；这条命令最后一次读正文发生在「删除文字被藏起来」的视图里，不补这一刀光标就一直画在左边一个删除段的宽度上（实测 −2142 twip），用户再敲一个字才弹回去——用户报的「光标经常左右横跳」就是这一左一右。回归 `npm run test:lowa-scroll`（同组现在兼管 caretX）。
...
- **`.uno:Escape` 会让顶层窗口退出全屏，别再派发它（v0.44.1 真机走查 D1，2026-09-15 真机 r5 无头探针）**：引擎在 `bootDoc` / `retarget` 里把 `getContainerWindow().FullScreen` 钉成 true（唯一出处 `ensureFullScreen()`），画布里才只有正文；`.uno:Escape` 把它退回去，VCL 于是画出自己的标题栏（「未命名 1 — ZetaOffice Writer」）和边框，正文缩在那个小窗里（实测容器窗口 800x576 → 785x691）。**事后补救补不准**：退出全屏**异步生效**（派发它的那条命令里读回 `FullScreen` 仍是 true、几何还没变，下一条命令才看到，实测一轮 20ms 的 macrotask 就翻了）；`FullScreen = true` 只找回标志位（标题栏没了）、尺寸仍是 785x691；`setPosSize` 回画布尺寸自己又会把全屏退掉；`.uno:FullScreen` 是开关语义（在 true 上派发 = 退出全屏）。所以 `ui_command {name:'escape'}` 现在走 `deselect()`（`ctrl.getViewCursor().collapseToStart()`），`escape` 不在 `UI_COMMANDS` 那张 `.uno:` 表里了。
- **用户按的 Esc 按事件落点分两路，这条分法是真机实测出来的（同上）**：
  - 落在**画布**上（Qt 抢了焦点，典型是 Writer 自己的原生右键弹窗开着）→ 覆盖层一个字都不碰，让它继续冒泡。**引擎自己的按键处理是唯一关得掉那个弹窗的路子**：宿主一 `stopPropagation`（或改成派发 `.uno:Escape`）就出事——前者弹窗关不掉、压在正文上（lowa-e2e `context-menu.mjs` 组 4 抓到），后者弹窗关掉了但**选区被清空**（组 6 抓到，该组明写「Escape closes the native menu without clearing the selection」）。可见 **Qt 的 keydown 监听在冒泡阶段**，不是捕获阶段。
  - 落在**覆盖层输入框**上（正常打字状态）→ 引擎不管它（Chromium/Qt 不从 `<input>` 里抢键，实测这种 Esc 引擎选区一动不动；打字也不会双份进文档就是同一个原因），所以宿主自己发 `ui_command escape` 取消选区。
- **覆盖层的控制键在 document 捕获阶段还有一层代收**（`onDocKeyDownCapture`，v0.44.1 真机走查 D8）：Tab/Enter/方向键/删除键不一定落在覆盖层输入框上（Qt 会在某些操作后把焦点抢回画布，合成键事件也可能直接打在画布上——真机现象是「候选出现后按 Tab 没反应，只能用鼠标点」）。落在**画布或 document 本身**的按键交给同一个 `onKeyDown`，处理掉的再 `stopPropagation`（Esc 是唯一的例外，见上一条：画布上的 Esc 原样让给引擎）；**输入框自己的事件不碰**（它的监听器已经处理过，碰了就是处理两遍），宿主自己的 DOM 面板（写作辅助 / 即时审校 / 审阅气泡）里的按键一概不碰。
...
- **lo-relay 的非命令消息**（客体页 → 宿主：`open-url` / `modified` / `selection` / `evidence-drop` / `boot-log` / `cursor-context` / `semantic-writing-state`；宿主 → 客体页：`insight-sub` / `semantic-writing-panel`）。**有据续写那一对（dev-board#748）**：面板本体仍在客体页（`semanticWritingPanel.js`，要跟着正文光标与选区走），但画布右上角那颗固定的启动按钮已撤掉——唯一入口是 `EditorToolbar.vue` 里的「有据续写」（`editor.toolbar.semanticWriting*`）。客体按 `!session || !writable` 算出可用态后用 `semantic-writing-state {available, open}` 回报，宿主 `LibreOfficeEditor` 的 `semanticWritingAvailable/Open` 只认这条（**不本地乐观翻**，否则客体里的「关闭」/Esc 与按下态会漂）；宿主点按钮只发 `semantic-writing-panel {open}`。面板关着时根节点 `root.hidden` 整个 display:none，画布上不占像素也不截点击。换文档（`initWritingAssistance`）宿主先把两个位复位，等客体重新上报。回归 `tests/completion/semantic-panel.test.mjs` 与 `semantic-toolbar.test.mjs`。`cursor-context` / `insight-sub` 是「依据」窗格的正文联动（dev-board#182，契约在 doc-insight.md 的「前端」一节）：**默认关闭**，宿主 `LibreOfficeEditor` 的 `insightSubscribed` prop 置位才下发订阅，客体页此前一次 `get_cursor_context` 都不打。宿主下行走 `this._transportSend`（relay executor 只发命令，非命令消息要自己送），`ready` 时补发一次——webview 重建/换文档后客体页状态是全新的。
...
- **「提前确认」不许把 `composing` 和输入框一起清掉**（`zetaOfficeImeOverlay.js`，dev-board#606）：#600 让一条非空的 `isComposing===false` 插入事件提前上屏，但那一刻**浏览器/系统的组合往往还开着**（macOS marked text）。旧实现顺手 `composing = false` + `doCommit` 里 `input.value = ''`，于是同一次组合的后续 input 事件走「非组合」分支被逐字 `insert_at_cursor` 写进文档（真机：一行里混进 `wenjianzi` 这种拼音字母），keydown 守卫 `if (composing || ...)` 也随之失效，Backspace/Enter/方向键被抢走；清空 value 还作废了 Blink 的组合区间，正文位置没有 preedit、候选条停在旧锚点不跟光标。现行契约：`composing` **只**由 compositionstart/compositionend 翻转；提前确认只提交一次并置 `compositionCommitted`，**不动 composing、不清 value、立即 return**；`input.value` 由 compositionend 统一清；compositionend 按 `committedText` 前缀差量补提交（同串不重复、更长只补尾巴、完全不同整串补上），**不要**在 compositionupdate 复位 `compositionCommitted`（那会把 #600 反向踩回去）。jsdom 用例在 `tests/revision-view/ime-composition.test.mjs`，真引擎在 `tests/lowa-e2e/ime-commit.mjs` 的 `confirm-then-continue` 轮（还原病灶即转红：一轮里 3 次 insert_at_cursor 而不是 1 次）。
- **组合中的文字要用独立预览条显示**：覆盖层的 `<input>` 压在画布上，必须全透明（显字会与正文叠印），所以「输入过程」看不见。预览条不依赖光标映射，未点过画布 / 映射失败时照样可见。
- **输入框必须 `opacity: 0`，只靠 `color: transparent` 不算透明**（`zetaOfficeImeOverlay.js`，dev-board#606 复测未过那一条）：`color`/`caretColor` 只管字与插入符，浏览器给**组合中的文字**画的输入法标记（ImeTextSpan，真机搜狗是蓝色下划线，无头 Chrome 是一条高亮底）用的不是 `color`——于是中文组合期间正文光标后面凭空多出一根线，宽度约等于拼音串。`opacity: 0` 不动布局：输入框照旧持有焦点、`getBoundingClientRect` 照旧贴着光标（系统候选窗按它定位），`compositionupdate` 照常触发预览条。**不许改用 `visibility: hidden` / `display: none` 藏**——那会连焦点和 IME 一起掐掉。回归 `npm run test:lowa-ime-underline`（`tests/lowa-e2e/ime-underline.mjs`）（真引擎无头，组合期间对光标右侧那一条做像素恒等比较；基线里先用一次合成 `compositionupdate` 把预览条摆出来，否则它的投影会漫进取样区）。
...
- **`authorName` 是空串时，引擎按自己的兜底署名——zh-CN 语言包里就是「未知作者」**（`SwModule::GetRedlineAuthor` → `STR_REDLINE_UNKNOWN_AUTHOR`；真机实证 24.2.8-zhcn-r5）。这条兜底对**所有**用户发起的写入一视同仁：手打、工具栏、查找替换（原生 `replaceAll` 快路径与逐命中路径都一样）。所以看到「查找替换产生的修订作者是未知作者」时，**不要去 worker 里找「哪条路径漏设作者」**——`execCommand` 每条命令开头都 `setRedlineAuthor`，`find_replace` / `replace_selection` 一条也没漏（e2e 组见下）；病灶一定在宿主给的名字是空的。
- **宿主拿名字的地方有坑**：`currentAuthorName()`（`LibreOfficeEditor.vue`）原先只读 `utils/auth.js` 的 `getCurrentUser()`，那是 `uni` 本地缓存键 `checkba_user`，**只有登录页 `saveSession` 与个人中心 `setSessionUser` 写它**——桌面端 local-mode 免登从 `launch.vue` 直达工作区，整机没登录过就永远读到空串，于是用户本人的每一条修订都署「未知作者」。现取数抽到 `utils/editorAuthor.js`（`pickAuthorName` + `createAuthorNameResolver`，纯函数可单测）：缓存里没有名字就问一次 `/api/auth/me`（local-mode 免登下同样回 `displayName`，可能是本地化哨兵「本机用户」），一个进程只问一次、失败不重试。**仍不许回落 username**（Spec §6）。
- **文档装好之后补署名靠「不带 bytes 的 `load_document`」**（`resolveRedlineAuthor` / `pushRedlineAuthor`）：worker 的 `load_document` 头两行先记 `authorName`，字节为空即早退，所以它是既有的**只改署名、不动文档**原语（lowa-e2e 一直用它注入作者名）。真机复核过：正文一字不动、已有修订不增不减、**旧修订的作者不回填**。`selfAuthor` 要同时更新（审阅面板「我」那一桶与引擎署名必须同串，见 dev-board#377）。别为这件事去 `reloadFromBackend` —— 那会丢掉用户未保存的编辑。
- 回归：`npm run test:lowa-redline-author`（真引擎无头，`tests/lowa-e2e/redline-author.mjs`，12 项：空名字 → 兜底作者、只改署名不动文档、补发后新修订署用户名、`__agent` 切换仍在同一条命令内）；宿主接线 `tests/project-home/libre-redline-author.test.mjs`，纯函数 `tests/lowa-unit/editorAuthor.test.mjs`。
...
- 审阅装订线宽度不抖（dev-board#725）：`npm run test:lowa-review-gutter`（真引擎，`tests/lowa-e2e/review-gutter.mjs`）。300 段 / 120 条修订 / 零批注的夹具在默认内联视图下边打字边读 5 秒，`get_review_layout().sidebarWidth` 的取值集合大小必须为 1（且为 0）。防空断言：第一次读必须 `truncated` 且拿到几何的条目少于修订总数，否则测不到「未排版」那一档。
- 写作词库采集（dev-board#725）：`npm run test:writing-caret`（真引擎），另跑一档 `LOWA_E2E_DPR=1.25 npm run test:writing-caret`。它从真实正文抽出机构/人名/法规候选，是 `seedDocument` 唯一的端到端护栏——`mode === 'all'` 那道闸在的时候它是红的。
- 原生光标几何（dev-board#725）：`npm run test:lowa-native-caret`（真引擎，`tests/lowa-e2e/native-caret-dpi.mjs`），DPR 1 / 1.25 / 1.5 / 2 × 缩放 100%/194%，每档 20 次 `get_cursor_rect` 都要拿到 `nativeCaret`。单档复跑设 `LOWA_E2E_DPR=`。
...
## AI 审校（dev-board#547，形态改造 #723/#724，命名与开关归属 #749）

**两层，开关只管上面那一层（#749 维护者拍板）**：不花钱的规则检查（结构、编号、交叉引用、算式、证件号码）**始终静默跑**，没有开关；「AI 审校」在停笔后自动跑、扣 Credits、可以关。产品名统一为「AI 审校」/ "AI Review"（与「审阅」= 修订批注面板区分），审阅面板第五个标签、浮球、桌面菜单、命令表全用这一个名字。

`inlineReviewHost.js` 读当前 Writer 正文快照，修改后 **2.5 秒**防抖调 `/insight/review`（`deep:false`）。`deep:true` 有两条路：面板/浮球菜单的「立即 AI 审校」，以及**规则检查跑完后排的自动那一次**（`scheduleAi()`）。会话与 worker revision 双围栏拦住迟到结果；修改、换文档、销毁立即失效。同用户开关同步，独立于写作补全开关。默认计时器用箭头包装调用，不能以 `{set:setTimeout}.set()` 调浏览器原生函数（Illegal invocation，Node 单测捕获不了）；真实浏览器测试覆盖启动与销毁。

`zetaOfficeInlineReview.js` 是不落盘的 guest DOM，**只剩两件东西**（#723/#724 之前那个 380px 的 `.awd-ir-panel` 已删除——它压着正文且关不掉）：
- `.awd-ir-ball` 浮球（#749 起是一颗药丸：`.awd-ir-open` 主体 + `.awd-ir-more` 的 ▾，菜单 `.awd-ir-menu` 三项「开/关 AI 审校」「立即 AI 审校」「收起到边缘」，都是真 `<button>`，Esc 关闭并把焦点还给 ▾）：图标 + 未读计数，可拖、松手**靠边吸附**、位置持久化。**浮球不随开关消失**——开关长在它身上，藏了就没地方再打开；只有会话结束（`status:'disabled'`）才整个不挂。**「收起」是贴边、不是隐藏（dev-board#866）**：原先「隐藏正文浮球」之后唯一的找回入口在右栏面板里，面板关着的人找不回来。现在 `hidden:true` 偏好（协议字段与存储键沿用，老用户藏过的直接变成贴边）的表现是浮球本体不挂、在它所在那一侧的边缘留一截 `.awd-ir-dock` 把手（真 `<button>`，16x48，hover 变宽；左侧 `left:0`，右侧 `right:18px` 让开引擎纵向滚动条；纵向对着浮球中线，没拖过就在默认左下），点把手发 `preferences {hidden:false}` 展开回原位；收起态行旁 chip 一起安静，计数只在把手的 title 里。右栏「AI 审校」页顶栏同一个开关，文案 `collapseBall`/`expandBall`。**位置存 localStorage 不是 sessionStorage**——客体页每开一份文档就是一个新 webview（sessionStorage 每次重来），而浮球摆在哪是跨文档的本机习惯（同 `awd_sidebar_collapsed` 口径）；键仍是宿主下发的 `layoutKey`（`awd_inline_review_layout_<hash(userId)>`，按用户不按文件，不带文档内容）。拖动有 4px 阈值，拖完那一下的 `click` 不算点击（否则每次拖完都会顺手打开面板）。
- `.awd-ir-chip` 光标旁标记：保留原样（当前段落有几条）。

点浮球主体或点 chip 都只发一条 `inline-review-request {action:'open-panel'}`，由宿主打开右栏审阅面板并切到「AI 审校」页；客体页不再自己画清单、不自己发 `deep`/`preferences` 之外的任何东西。浮球三态：AI 跑着 `.busy`（title「AI 审校中」）/ AI 关着 `.off`（title「AI 审校已关闭（规则检查照常）」，**计数照显示**，规则那一层还在跑）/ 自动被额度·限流·模型·地域停掉时 title 直接说原因。**藏浮球（`hidden:true`）时正文里一个字都不挂**——「已关闭」的提示本身就是打扰。IME/编辑/滚动立即隐藏旧定位；**LOWA boot 每秒发同尺寸 synthetic resize，不能因此清掉提示；只有视口或 canvas 几何变化才失效**。不抢 Tab；候选菜单打开时只让 chip，不动浮球。

清单在宿主：`components/InlineReviewPanel.vue` + 外置 `inline-review-panel.scss`（壳-芯分法同 EvidencePanel），挂在 `ReviewPanel.vue` 的**第五个标签「AI 审校」**（key `chk` 与 i18n 键 `editor.review.checkTab` 都没改，`v-show` 常驻以便标签显示计数；`inlineReview` prop 为 null 时整个标签不出现）。分类固定为全部/待补充/一致性/格式与号码/AI 审校，**计数恒按未筛选全量算**；判定全在纯函数 `utils/inlineReviewGrouping.js`（`bucketOf`/`filterByBucket`/`countByBucket`/`visibleFindings`/`isFresh`/`isLocatable`/`isApplicable`），**guest 与宿主共用这一份**——各写一份的话浮球上的数字和面板里的条数会对不上。定位走 `goto_review_range`、采用走 `apply_review_edit`（都由面板直接发给 executor），**禁止 `find_text_locations`**（会把书签写进 docx）。正文一改宿主就把 findings 清空（旧坐标不可信），面板**保留上一轮清单**并标「等待重新检查」+ 禁用定位/采用（清单整片消失再冒出来，律师会以为问题自己没了）。

开关（**默认开启但安静**，不是默认关）：**工具栏上已经没有「审校」按钮了**（#749 撤掉，`toggle-inline-review` / `inlineReviewOn` / `inlineReviewAvailable` / `toolbar.inlineReviewShort` 一并删除，别再加回去——开关要和它控制的东西待在一起）。入口只剩三个：浮球的 ▾ 菜单、审阅面板「AI 审校」页顶栏、命令表 `doc.aiReview`（`wb:toggleAiReview` → `LibreOfficeEditor.menuToggleAiReview`，`menuState()` 上报 `aiReview`）。

**偏好换了新键 `awd_ai_review_<userId>`**（用户级、跨标签共享，**不按 fileId 拆**），值是 `{ai, hidden}`：`ai` 只管 AI 那一层（关掉之后一次模型调用都不发，规则检查照跑）；`hidden` 只管浮球。旧键 `awd_inline_review_<userId>` 的 `enabled` 语义是「连规则一起关」，对不上，所以**只在新键缺席时读一次做迁移**，且只认一个方向：`enabled:false` → `ai:false`。**绝不替一个明确关过这功能的人重新打开一条要花钱的链路**；关着这件事在浮球 title 与面板开关上都写着，不是偷偷关的。`layoutKey`（浮球位置）没改，仍是 `awd_inline_review_layout_<hash(userId)>`。

自动触发（`scheduleAi()`，三道闸缺一不可，改之前先想清楚每次自动跑都在花用户的钱）：①停笔 `AUTO_AI_IDLE_MS = 20s`（规则那一层是 2.5s，它不花钱）；②两次**自动**之间至少 `AUTO_AI_MIN_GAP_MS = 3min`（只有空闲判据的话，边写边停每 20 秒就是一次整篇调用）；③**正文哈希没变就不跑**（`deepHash === cachedHash`，与 #724 那条快照哈希同一把尺子）。另外 `!ai / !active / !writable / deepBusy / autoBlocked` 任一成立都不排。只有规则检查成功跑完那一处调 `scheduleAi()`——AI 失败了不在原地自动重试，要等正文真的又变、规则再跑一轮。`AUTO_AI_BLOCKING`（`DEEP_QUOTA` / `DEEP_RATE_LIMITED` / `DEEP_MODEL_UNAVAILABLE` / `DEEP_REGION`）出现即置 `autoBlocked`，本会话不再自动跑，手动按钮仍在。

**送的是整篇，不是增量**：同一条 POST 里的规则检查是跨段落的（交叉引用、前后数量、算式），只送改动段会让它们算错，而返回的 `findings` 是整份清单不是补丁——增量要动后端的检查口径，不在 #749 范围。按 `insight.chunk-chars=10000` 估，整篇 50 页中文合同约 3-5 个 chunk、每 chunk 上行 ~7k tokens。
...
IME 光标定位优先用 `XController.getViewData()` 的实时分号数据与 VCL 编辑子窗坐标；该格式不是 UNO 公共字段契约，当前只对锁定的 LOWA 24.2.8-zhcn-r4 验证，解析或窗口匹配失败必须退回点击校准——但**点击是最后一条兜底，不是第一条**：`computeRect()` 每次原生读取成功都顺手把 `anchor`（实时原点偏移）按原生几何重新推出来，所以下一次窗口匹配失败时映射的是真光标几何而不是一次陈旧的点击（dev-board#725）。窗口按「可见区尺寸」在窗口树里认，期望尺寸由 twip 经实时 DPI 换算而来、与窗口自己的整数像素尺寸之间有舍入误差，**容差要随 DPI 放大**（`Math.max(2, Math.ceil(2 * dpi / 96))`，96dpi 下仍是 2，125%/150% 是 3，2x 是 5），固定 ±2 物理像素在分数 DPI 上会认不出编辑子窗。回归 `npm run test:lowa-native-caret`（`tests/lowa-e2e/native-caret-dpi.mjs`，DPR 1 / 1.25 / 1.5 / 2 × 两档缩放各读 20 次）；**注意：无头 Chromium 的 DPR 模拟在 macOS 上四档全部 0/20 失手，即原容差本来就够用，这条容差放宽属未复现的硬化，Windows 真机未验证**。VCL 窗口尺寸使用物理像素，必须用 component 的 `convertPointToPixel` 读取实际 DPI；固定 96 会使 Retina 匹配失败。worker 坐标含 component/child 偏移，宿主再补 canvas 与 container 的实时菜单高度差（隐藏菜单时为 0）；CSS 宽度比负责 DPR/缩放换算。100%/194% 长文档验证已覆盖。异步 `get_cursor_rect` 必须只允许最新请求落位，避免旧响应覆盖新光标后把补全菜单带回旧位置；缩放、滚动后的下一次输入或移动重新读取实时几何。
