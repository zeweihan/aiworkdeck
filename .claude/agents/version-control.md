...
- `WorkSessionBar.vue` —— 顶部状态条，三态：`onDraft` 非空时是稿态「正在稿《{name}》上修改」+「回到主稿工作」「采纳这一稿」「放弃这一稿」三个按钮；`working` 时「工作中（已改 N 份文件）」+「结束本次工作」「丢弃」；都不是则空闲态固定文案「当前没有进行中的工作」（**注意：全仓没有「主线」这个用户可见文案**，命名弹窗走 `.awd-dialog` + uni-app 的 `.uni-input-input`，不是外层 `.awd-input`）。稿态的 `.draft-dot`/`.session-idle` 是 e2e 断言认组件真渲染的独有选择器（不用 body innerText 包含）。
...
- 后端触发点：`ProjectFileService.signalChange()`与 `FileController.signalChange()` 两处调用 `workSessionService.onChangeSignal(...)`，都用 try/catch 包死、绝不阻断文件操作/上传。IDE 化本地文件夹项目（`Project.localRoot` 非空）另有 `LocalRootWatchService`（FSEvents 监听 + 防抖 800ms）触发 `LocalProjectService.reconcileProject` 对账——对账只写数据库、绝不写磁盘，新建/更新/软删除走 ProjectFileService 服务方法，signalChange 因此自带，**Finder 里的增删改同样进版本记录**。对账三条硬规则：无变化不动行（防版本噪声与修改时间失真）、回收站行不复活（软删除不动磁盘）、根目录不可达整体跳过（防外置盘拔出误判成全删）。
  - **反过来，应用里对文件夹做的事必须落到磁盘上（dev-board#885）**：对账以磁盘为真相源，「行在库、目录不在盘」就判成律师在 Finder 里删了它。`ProjectFileService` 在本地文件夹项目里：新建/恢复/复制/会话文件夹都 `materializeLocalFolder` 建目录；文件夹改名/移动走 `relocateLocalFolder` 整体搬目录并改写子孙 `filePath`（搬不动就抛错回滚，大小写改名借临时名过渡）。此前这几处只落库——空文件夹在下一次任意磁盘变化（新建文档、保存、写 `.awd/`）触发的对账里必进回收站、恢复后再进；改名被对账撤回并连子孙一起送进回收站；移动在原位置导回幽灵。唯一例外是根下的 `__staging_area__`（`ProjectFileService.isRootStagingFolder`）：不建空目录、对账的删除同步也跳过它。解析器是字段注入（`setStorageResolverForTest`），手工 new 的实例里整段跳过。护栏 `LocalProjectServiceTest` 的五条 #885 用例。
...
**只读的 AI 提问不开工作段（dev-board#822）**：`commitAiRound` 里的 `ensureSession` 存在的理由只有一个——让 AI 那笔自动存档落进一段工作里（时间线按工作段分组），**不是**「问一句 AI 就等于开了一段活」。#218 落地时它是无条件调用的，于是律师只问一句纯只读的话（本轮只调了 `doc_list_project_files`），案卷也进「工作中」，随后「取回最新稿」「交稿」被 `CloudSyncService.requireCleanForCloudOps` 当场挡掉——v0.46.3 发版门的 app-e2e J14-准备就是这么确定性红的。现在 `commitAiRound(projectId, userId, documentEdited)` 的判据有两条，任一成立才开段并落版：① **`documentEdited`**——编排器传进来的「本轮成功调过编辑器桥写入工具」（`RunGuard.documentEdited()`，判据是 `ClientCapabilityService.isDocumentWritingTool`，与 `bubble_end.documentEdited` 同一个标志）。**这一条不能省**：`doc_*`/`sheet_*`/`slide_*` 的改动先落在前端编辑器里，字节要等自动保存才回到服务端，落版这一刻工作区还是干净的；② **工作区真的脏了**（`repoService.pendingChanges` 非空）——服务端直接落盘的那些工具（`write_file` / `text_*` / pptx / 导出类）走的是这条，它们本来就各自 `signalChange` 过、段早开好了，这一条兜住的是「脏但无段」的路径，顺带覆盖只动了文件树清单的软删除（所以 `writeToWorkTree` 必须排在判据之前）。读取待提交变更失败时按「有改动」处理——版本记录是保险，宁可多开一段也不能漏掉一次落版。护栏：`WorkSessionServiceTest.readOnlyAiRoundDoesNotOpenAWorkSession` / `.aiRoundThatWroteFilesStillOpensASessionAndCommits` / `.editorBridgeWriteOpensASessionEvenBeforeTheBytesLand`，加编排器那一侧的 `AgentOrchestratorDocumentEditedFlagTest.readOnlyTurnTellsVersionRecordingNothingWasWritten` / `.writingTurnTellsVersionRecordingToOpenASession`（后两条钉的是「编排器到底传了哪个值」，与服务层判据分开各守一半）。
...
**改动信号查不到人时退到项目负责人**（v0.49.0 BUG-59）：诉讼可视化等 AI 路径落盘带占位 userId 10001
（`AGENT_USER_ID`，库里没有这一行），`ProjectFileService.signalChange` 以前回落泛称「用户」，这一笔成了工作段
最后的操作者，30 分钟空闲自动结束时整段署成「用户」且不认作「你」。现在 userId 与署名一起换成项目负责人，
两者都查不到才回「用户」。护栏 `service/ProjectFileServiceSignalActorTest`。
...
- **「已保存，尚未存为版本」（`provenanceFileUncommitted`，2026-09-25 前文案是「本机未保存的改动」，dev-board#933 改掉：保存成功后仍说「未保存」会让律师以为文件没存）= `ProjectRepoService.isPathDirty`**：**单路径 `git status`，只读、一次 `add` 都不做**
...
6. **界面零 Git 术语**——包括中文直译。「主线」也算（是 trunk 的直译），空闲态文案已改成「当前没有进行中的工作」（`WorkSessionBar.vue`）。界面上与「稿」相对的那条线一律叫「主稿」（v0.49.0 BUG-56：状态条「回到主稿工作」、完整历史行徽标、合并两侧称呼、后端 `mainline` 引用名都改了），完整历史标签名与入口同为「完整历史」，行与详情都不显示短哈希、对比两版的两侧称呼用「日期 时间」；护栏 `frontend/tests/version-history/historyNoGitTerms.test.mjs` 扫 zh/en 两份 `version.js` 的全部值。改文案前 grep 一下现有组件模板，不要凭直觉猜产品文案；`frontend/tests/app-e2e/run.mjs` J9 段的注释记录过一次「brief 假设的文案与实际组件不符」的教训。
