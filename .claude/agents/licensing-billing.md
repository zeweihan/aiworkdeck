...
40. **会议录音的告知是「告知」不是「同意」，强度与跨境那条刻意不同**。
    ...
    **不出境**，触发的是告知义务。**照抄那道硬闸的代价是律师录完两小时会、点转写时
    被模态框拦住**——最差的打断时机。所以主把关在
    `MeetingRecordingPanel` 的录音键与「开始转写」两处，摆在**录音开始之前**
    （唯一一个什么都还没发生的时刻）。

    但**服务端也有一道，只是它是「一次性」不是「每次」**：
    `MeetingTranscriptionService.recordingNoticePending()` 在提交转写那条缝上查一次
    （仅 platform 档——byok 走用户自己的 OSS 与听悟账号、local 不出本机，两者都没有要告知的事）。
    这道闸不能省，因为**真正的上传时刻不是「点转写」而是录音结束**：
    `POST /api/meetings/{id}/finish` 会自动提交，常见路径下用户在上传那一刻没有任何动作。
    没有它，任何绕过面板的调用方（未来的插件/命令、崩溃恢复补刀）都能不经告知把一场会传上来，
    而**被录的是第三方——他不是我们的用户，没同意过任何条款**。
    它仍然不是「每次硬闸」：确认过一次就永远为 false（除非告知改版），
    正常路径下用户在录音之前就确认过了根本撞不到；会被拦住的只有
    「从没看过告知、却有东西替他上传」这一种，那正是要拦的。
    三条会在重构里失守的事：
