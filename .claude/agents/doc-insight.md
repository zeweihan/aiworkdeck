...
| UNAUTHORIZED | `Kind.UNAUTHORIZED`（账户 Key 无效/被吊销）；**以及上游把 tools/call 当无凭据驳回**（法宝原话 401 + `900902 Missing Credentials`，dev-board#688 D3） | 「去连接账户」/ 自动补全资料卡里是「去设置配置」 |
...
判定与文案在**依据窗格与自动补全资料卡之间是同一份**：卡片的失败态也只看 `retrievalHint`。
