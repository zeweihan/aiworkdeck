...
**`context_notice`（本轮附件被怎么处理了，dev-board#801 K21 ⑦）**：载荷 `{"kind":"truncated|dropped|ocr_fallback|image_limit|image_too_large|unreadable","fileId":"9","name":"合同.docx","detail":"50000"}`（`name`/`detail` 可缺省）。生产者只有 `ContextAssemblerService` 经 `ContextTurnSink`，发生在 assemble 期间（首 token 之前）。
- **六个 kind 不许合并**：它们对用户是六句不同的话，处置也不同——该换模型的（ocr_fallback）、该少贴几张的（image_limit）、该压缩图片的（image_too_large）、该删掉几份材料的（dropped）、该知道只读了前 N 字的（truncated）、该知道这份根本读不出字的（unreadable）。合成一句「未能处理」等于什么都没说，那正是改造前的状态（三条降级路共用一句「超出本轮张数或单张体积上限，或读取失败」）。
