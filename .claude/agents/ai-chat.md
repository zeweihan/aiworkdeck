...
- `controller/ai/AiModelCatalogController.java` — `GET /api/ai/models`（鉴权口径同 AiChatController：X-Session-Id → userId，null 则 401）。响应契约：`{networkRegion, networkRegionMode, networkRegionBasis, defaultModel, priceDisplay, models:[{id,name,vendor,region,contextLength,**vision**,inputPricePerM,outputPricePerM,tiered,displayInputPerM,displayOutputPerM,priceLevel}]}`（后三个字段与 priceDisplay 是 dev-board#853 加的，见下文「模型价格显示口径」）。models 只含 `AllowedModels.availableIn(effectiveRegion())`；价格取首档，`tiered=true` 表示有分档、UI 要提示「长上下文单价更高」。defaultModel 必须由 `ChatModelFactory.resolveDefaultModel()` 解析（DB `ai.defaultModel` 优先于 yml），前端自己挑「清单第一条」会和实际发出去的模型不一致。刻意不放进 AiChatController——那是被治理过一轮的胖控制器，模型目录与对话没有共享状态。响应契约由 `AiModelCatalogControllerTest` 守（2026-08-29 新建；此前 `ChatModelFactoryTest` 有一段注释声称「护栏在模型目录端点的测试里」，**而那个测试根本不存在**，端点契约一直零覆盖——别再相信那条注释）。
...
- **前端消费侧**（改模型选择器前先看这三条）：① `frontend/src/components/ChatInterface.vue` 的模型清单来自 `GET /api/ai/models`（`api.js` 的 `fetchAiModels()`，同一端点只有这一个函数名），下拉本体是共享组件 `components/ModelSelectorDropdown.vue`（空态向下、对话中向上两处都用它，dev-board#853 从两份重复模板抽出），按 vendor 分组、`region=INTERNATIONAL` 的组排在最后并标注「需国际网络」、`tiered=true` 显示「长上下文单价更高」、每条显示首档单价（口径见下节）；默认选中项取响应里的 `defaultModel`，**不许自己取 `models[0]`**。② 模型选择持久化在 uni storage 键 `ai_selected_model`（全局非按项目）；恢复时必须校验该 id 仍在端点返回集合里，不在则回落 `defaultModel` 并提示一次——AI 面板挂在 `v-if` 上，不落盘会静默复位，而这个选择有计费含义，静默换计价对象是本次要修的老毛病。③ `provider=OLLAMA` 时模式选择器只留 ASK（本地档不支持工具调用），判据取 `GET /api/ai/config` 的 `activeProvider`（模型目录端点不回 provider）；该字段现在由 `ChatModelFactory.resolveProvider()` 透出，与真实路由同源。
- **模型价格显示口径（dev-board#853）**：选择器每行只写两个数（前输入、后输出，每百万 tokens），单位/口径/档位含义统一写在下拉底部脚注，和「网络判定」合成一个脚注区。
  - **口径二选一，由后端定**：`priceDisplay.basis = charged`（实付价）只在 **provider = AWD_CLOUD 且取到了官网扣费汇率**时成立——`display* = 美元首档单价 × exchangeRate × marginMultiplier`，站点币种（国内站 CNY、国际站 USD）；其余一律 `list`（厂商美元标价，`display* == inputPricePerM/outputPricePerM`，factor=1）。**自备 Key（OPENROUTER）绝不乘平台汇率**：那档用户按标价直接付给 OpenRouter，乘上去就是报一个他不会付的价；本地 Ollama 同理。`priceDisplay.channel`（platform/byok/local/unknown）只给脚注措辞用。原有 `inputPricePerM/outputPricePerM` 不删不改名，它们就是「厂商标价」这个事实。
  - **汇率来源**：`service/ai/ModelPriceDisplayService` 读既有的 `GET /api/account/ai-usage`（`AccountService.fetchAiUsage()`，**不新增出站请求**）的 `exchangeRate` / `marginMultiplier`，以及官网新版才有的 `currency` / `exchangeRateSource` / `exchangeRateUpdatedAt`。缺 `currency`（旧版官网）时按汇率推断：|rate−1|≤0.05 判 USD（国际站官网把汇率钉成 1）、>3 判 CNY，中间地带不猜。**不许编造汇率**：字段缺失/非正/币种不认识/未连接账户/官网不可达，一律退回 list 并在脚注写「厂商美元标价，实际按 Credits 扣费（暂未取到账户扣费汇率）」。
  - **不许拖慢下拉**：取数在单线程后台池里做，`/api/ai/models` 最多等 `ai.model-catalog.price-rate-wait-ms`（默认 1200ms），等不到先用同账户旧值、没有就 list，后台那次完成后写缓存。成功保鲜 10 分钟、失败 60 秒，按账户指纹记（换账户自动作废）。刻意**不和 `PlatformCreditsGate` 共用缓存**：那边是「首查同步阻塞 + 60 秒」，服务的是零余额当场拦住。
  - **贵贱档位 `priceLevel`（1 经济 … 4 旗舰）**：`AllowedModels.priceLevel()`，综合单价 = 0.8×输入 + 0.2×输出（首档，美元；agent 每轮重发 system+工具规格+历史，输入占绝大多数），阈值 `PRICE_LEVEL_UPPER_BOUNDS = {0.3, 1.0, 3.0}`（约 3 倍一档）。**按美元算**，两站档位一致。硬约束是境内只剩 GLOBAL 9 条时四档仍都有模型（今天 2/3/3/1），`AllowedModelsPriceLevelTest` 守；改价或加模型后它红了就调阈值，别删用例。**前端不许再算一份档位**，缺字段就不分档。
  - **前端**：格式与口径的纯函数在 `frontend/src/utils/modelPricing.js`（数字：<1 两位有效数字、<100 一位小数、否则取整，**不去末尾 0**，否则一列数字对不齐；`tests/project-home/model-pricing.test.mjs`）。视觉层级只用墨色一族令牌（text-3 → text-2 → text 500 → text 700）加一个 4 格小条，**不用红绿、不用竹月青**（2.6:1 承载不了文字），价格列 `tabular-nums` + 固定最小宽度右对齐，保证各行的「/」在一条竖线上。档位名称只在悬停提示（`title`）与脚注里出现。
  - **地雷**：下拉挂在输入栏工具条里，祖先有 `white-space: nowrap`，下拉根元素必须重置成 `normal`，否则脚注整句不换行、撑出横向滚动（这次真渲染走查抓到的）。
  - 验证：`mvn test -Dtest='AiModelCatalogController*,ModelPriceDisplayService*,AllowedModels*'`；`npm run test:project-home`；真渲染 `node tests/chat-presentation-ui/model-pricing.mjs`（合成 `/api/ai/models`，两种口径 × 两个展开方向 × 中英 + 窄面板，截图落 `AWD_SHOTS`）。
...
- **附件跨轮保留（K14 ③，审查 E-2）**：发送后 `contextFiles` **不再被过滤掉**，`restoreCarriedTags` 把标签重新挂成 `is-carried`（淡态 + 虚线边）。**淡态仍然会继续带上**——这才是 E-2 要的；点 × 移除，点标签本体确认沿用（`confirmCarriedFile`，只是摘掉淡态）。**可发现性（BUG-19，v0.49.0 真机测试 C5-04 用户看不懂虚线）**：淡态标签的 × 常显不等悬停，`title` 走 `chat.carriedAttachmentTitle`；输入框里还挂着淡态标签时（`carriedHintVisible`）卡片上常驻一句 `chat.carriedAttachmentHint`「上轮附件，下一条继续使用，点 × 移除」。**跨轮携带是刻意设计，别再改成发后即清**（那一版被复核撤回）。粘贴图仍然发后即清（已上传成项目文件，要继续用从文件树拖回来）。配套守卫：**「只剩上一轮带过的附件、没有任何新内容」算空消息**，否则发完一轮误按一次回车就会把同一批材料顶着空 prompt 再发一遍。
...
- **取消真正生效（2026-09-22，计划 K4；审计 C-03 / D-01 / D-10）**——改造前「停止」只置一个布尔，真机实测点完还要跑 82 秒（`Cancelling conversation` 与 `Response completed` 相隔 82 秒），卡在桥调用上要等 180 秒。四道闸**缺一条停止就还是「按了要等」**：
  ...
  - **落库**：`handleCancellation` 现在带 `StringBuilder executionLog` 形参（四个调用点都在 runLoop 内，直接传实参），落 `logText + partialContent + "[已中断]"`（这个尾标记留给模型下一轮读历史用；前端 `parseAssistantHistory` 回灌时按 `HISTORY_INTERRUPT_TAIL` 把正文**最末尾**的中断标记摘出来改走 `stopNotice` 状态标签，复制/用到文档不带它，BUG-42；后端标记字面量改了要同步那张表，契约断言在 `chat-timeline-contract.test.mjs`），条件是**两者任一非空**——与 `finishWithError` 逐字对齐。**同源的 `handleStreamErrorTerminal` 一并补掉**：它原先给 `finishWithError` 硬传 null executionLog，于是「模型先调了几个工具、然后这一轮流式出错」的收尾同样把过程卡整段丢掉；现在它也带 `executionLog` 形参（六个调用点作用域里都有实参，**没有一个需要传 null**）。护栏 `AgentOrchestratorTerminalErrorPersistenceTest.streamErrorPersistsTheExecutionLogOfToolsAlreadyRun`——注意它的两条断言必须打在**同一行**落库内容上：整批工具跑完后那次增量保存会先写下过程卡，分别看两处都「有」是假绿。**真正会丢的是「取消落在工具批次中间」那一档**：整批跑完之后有一次增量保存会捎上执行日志，而逐个工具前的检查点直接 return，那次增量保存整段跳过——形态就是「AI 已经动过文件、用户点了停止、刷新后历史里什么都没有」。
...
- **对话按时间线分层（2026-09-15，dev-board#646，取代 #584 的固定活动面板）**：`ChatInterface` 仍用 `AgentMessage/chatTurns.mjs` 按 USER 分轮（`.conversation-turn` + `.message-row.user/.assistant`，保留原 bubbles 与全局索引），但思考/执行/任务/产物/正文**全部回到消息流里按发生顺序渲染**，顺序由 `AgentMessage/chatTimeline.mjs` 决定。`TurnActivityPanel.vue` / `TitleCard.vue` / `RootBubble` 的 `hideActivity` / 固定活动面板入口 / 28 个 `chat.activity*` 文案键（只留下 ChatInterface 还在用的 `activityBackToLatest`）**已整体删除**；「待回答/审批的常驻定位入口」以浮条形态补回（dev-board#663，见下条）。后台任务取消仍走输入区与 BackgroundTaskIndicator，不混用生命周期。
  ...
  - **围栏剥离与分片不变式**：`processTextStream` 现在每轮都把 `parserBuffer` 抽干到只剩半截标签，所以 ```` ``` ```` 的剥离**必须放过「结尾还可能长成完整围栏」的那截**（`PARTIAL_FENCE` = /`{1,3}[A-Za-z]*$/，与半截标签一样留在缓冲区里；流结束时由 `flushRemainingBuffer` 补剥）。不放过的话跨分片的围栏永远拼不起来，````xml```` 原样漏进正文——**同一段文本按不同分片得到不同正文**，而且不报错。历史回灌（`processTextStream(content, true)`）恒不剥离：历史正文里的代码块是真的。流式剥离也**只剥协议外壳**（裸 ```` ``` ```` 或 ````xml/html/markdown````），正文代码块（````json````、````js````……）连同收尾围栏原样保留，`fenceState.open` 跨分片记住「正处在正文代码块里」（BUG-17）。**带状态就必须单次处理**：每段文字只许过一遍剥离器，`fenceState.pending` 记 `parserBuffer` 结尾还没剥过的那截（PARTIAL_FENCE 扣下的半截围栏），`processTextStream` 只剥「pending + 新到的」、`flushRemainingBuffer` 只剥 pending；因半截 `<` 留在缓冲区的尾巴是剥过的，再剥一遍会把收尾 ```` ``` ```` 当成新开的代码块、后面整段渲染成代码且随分片变（复核实测）。护栏里的「代码块含 `<`」两条样本专测这个。护栏 `tests/project-home/chat-timeline-contract.test.mjs`（同一输入按 1/3/9999 分片必须逐字相同）。
  ...
  - `useChatReadingPosition` 观察实际消息内容高度（ResizeObserver），仅在 followLatest 时贴底。向上阅读/定位历史暂停跟随，点击“回到最新”恢复；禁止重新加 bubbles 全树 deep watcher。**「回到最新」与待处理定位钮住在 `.return-to-latest` 这条常驻 36px 留白带里（消息列表之下、输入区之上，有消息就在），不许再做成贴列表底边的浮层**（dev-board#870：浮层会压住过程卡/正文）；也不许改成「按钮出现才占高度」——列表高度一变会夹紧 scrollTop、触发 scroll 把 followLatest 打回 false，按钮来回闪。守卫是 `run.mjs` 的 `locatorOverlaps`（按钮与列表可视区内露出的行/过程卡不相交、320px 不撑出窗格，浅深两主题各截一张 `k870-back-to-latest-{light,dark}.png`）。导航仅滚动本聊天容器。
...
**向用户提问 `ask_user` 工具（dev-board#868，对标 Claude Code 的 AskUserQuestion）**：与上面的 `<question>` 标签并存，是「拿不准先问」的**首选**入口（结构化选项、可多选、界面自带「其他」自由输入）。病灶：「你能帮我清理已经打开的这个文档么」——疑问句、「清理」没说标准——模型读完全文自己判定哪些是「混入的审查报告」开删。
- **一份口径**：`service/ai/AskUserQuestion`（参数校验 / SSE 载荷 `toEventJson` / 落库标记 `toMarkup` / 回答识别 `isAnswerMessage`）。工具本体 `tools/AskUserTools.ask_user(question, options?, multi_select?, header?)` **只做校验**（2-4 个选项或不给、标签去重、模型自写的「其他」静默丢掉，不合格回 `Error:` 让模型改参数重发），停机由编排器做。
- **编排器接线**（`AgentOrchestrator`，原生与 XML 兜底两条分支都接）：分发成功后 `askUserFrom(name, argsJson)` **按参数**重建问题（不读工具输出——回放评测里工具输出是桩），过程卡那一行的输出换成人话 `askUserProcessNote()`，**同批后续工具一律不执行**（紧跟着的写入正是还没被授权的动作），然后 `stopForAskUser`：text_delta 流出标记 → SSE `ask_user` 事件 → 落库（执行日志 + 标记）→ 复用 `stopForUserQuestion`（AWAITING_INPUT、`bubble_end status=awaiting_input`、不递归、不跑记忆管线）。**顺序固定：标记在前、事件在后、bubble_end 最后**（`AskUserOrchestratorFlowTest` 钉住），前端靠它让事件覆盖解析器从标记里拼出的那份。参数不合格（`askUserFrom` 返回 null）按普通工具回喂、不停机。
- **标记格式** `<question kind="ask_user" id="ask-…" header="…" multi="true">问题<option description="…">标签</option></question>`：属性值实体转义（前端 `decodeAttr` 还原），正文与标签只中和协议标签形状。它同时服务三件事：历史回灌（桌面解析器读 kind/id/header/multi/description）、**Office/WPS 任务窗格**（插件只认文本流里的 `<question>`/`<option>`、忽略属性，于是不改一行也能显示问题与单选按钮——但**没有**说明、多选与「其他」，点选项发的是选项原文而不是回答标记，这是已知降级）、模型下一轮读到的历史。`containsQuestion` 天然认得它。
- **SSE 事件 `ask_user`**：`{"v":1,"id","question","header","options":[{"label","description"}],"multiSelect"}`，`v` 只加不改。前端 `useAgentStream` 在气泡守卫之前处理（断线补发时气泡指针可能为 null），整块覆盖 `bubble.question` 但保留已作答状态。
- **回答回填**：前端 `utils/askUserAnswer.mjs` 的 `formatAskUserAnswer` 拼 `<ask_user_answer id="ask-…">\nQuestion: …\nSelected:\n- 标签\nOther: 补充\n</ask_user_answer>` 作为下一条**普通用户消息**的 `message`，`displayText` 只放所选项（契约 D）。**后端不新增 DTO 字段**，只在 `ContextAssemblerService.clarificationReminder` 里按 `isAnswerMessage`（只认开头）把末位提醒换成「这是对 ask_user 的回答，按回答继续，别再问同一件事」。改回答格式必须两边一起改（`ask-user-answer.test.mjs` 读 Java 源码对拍标签名）。
- **提示词三处**：enforcement 的 Stop Conditions 加「ALSO STOP once you call ask_user」、Output Structure 的 `<final>` 例外加 ask_user；AGENT 模式约束第 1 条加「要求本身含糊时也先问 + 指令明确就直接做」；基底 prompt 的 Clarification 一节改成 `ask_user` 首选（中英两版同步，含「清理」例子与「删第 12-21 段」反例）。**真正起作用的是末位**：`clarificationReminder` 只在 AGENT 模式挂到用户消息最末尾（排在活跃文档提醒之后），三条判据 + 反例 + 「用户回答前不要改文档、不要写任务清单」。2026-09-23 真实模型实测（deepseek-v4-flash、温度 0.7、合成 30 段认购协议）：末位只写一句泛泛的规则时「清理」5 次都先问了、但 3 次用的是旧 `<question>` 标签；点名「调 ask_user 工具、不要只写 <question> 标签」后 5 次里 4 次走工具、1 次先写任务清单然后直接开删；最终版（三条判据 + 句末「吗/么/？」+「回答前不要改文档、不要写任务清单」）8/8 走 ask_user，「把第 12 到 21 段删掉」8/8 直接执行、0 次反问。**改这句末位提醒之前先重跑 `AskUserLiveEvaluationTest`**——措辞的细微差别就是上面这几档的差别。
- **可见性**：`SkillRouter.ORCHESTRATION_TOOLS` 与 `ToolDisclosurePolicy.CORE` 都含 `ask_user`（skill 白名单与渐进披露都裁不掉）；三档客户端能力都可见；**子 Agent 里排除**（`SubAgentService.resolveScope` + `executeScoped` 双拦：子任务没有用户可问，也停不了父轮次）；ASK 模式不下发工具，自然不可用。不经编辑器桥，不进 DOC_ACTIONS / PLUGIN_SPEC / pluginDocActions / check-tool-parity 四处清单。
- **前端问题卡** `AgentMessage/QuestionCard.vue` 的 `kind="ask_user"` 分支：选项是真 `<button>`（单选点一下即作答；多选勾完再「发送回答」），「其他」点开才出现的单行输入（uni input 的 100ms 节流用 DOM 值兜底、IME 组合回车不提交、只发一次），作答后整卡只读并按回答高亮（live 由 `handleQuestionAnswer` 就地写 `question.answer`，历史由 `markAnsweredQuestions` 用 `parseAskUserAnswer` 读下一条用户消息）。旧 `<question>` 标签的卡片形态一行没动。样式只用 `--awd-*`，选项有 0.15s 底色过渡（截图/取色要等它走完）。
...
- **第二个消费者在版本记录那一侧（dev-board#822）**：`Loop Finished` 分支把同一个 `guard.documentEdited()` 传给 `workSessionService.commitAiRound(projectId, userId, documentEdited)`，它是「这一轮该不该给案卷开一段工作」的两条判据之一（另一条是工作区真的脏了，见 `version-control.md`）。**为什么非它不可**：`doc_*`/`sheet_*`/`slide_*` 的改动先落在前端编辑器里，字节要等自动保存才回到服务端，落版这一刻工作区往往还是干净的，服务端没有别的办法知道这一轮写过东西。反过来，只读轮（本轮只调了 `doc_list_project_files` 之类）两条判据都不成立，不开工作段——旧实现无条件开段，律师只问一句话案卷就进「工作中」，随后「取回最新稿」「交稿」被 `CloudSyncService.requireCleanForCloudOps` 挡掉，v0.46.3 发版门的 app-e2e J14-准备因此确定性红。所以**这个标志的准确性现在有两处后果**：报错会让「用到文档」按钮出错，还会让案卷的工作段状态出错。护栏 `AgentOrchestratorDocumentEditedFlagTest.readOnlyTurnTellsVersionRecordingNothingWasWritten` / `.writingTurnTellsVersionRecordingToOpenASession`。
