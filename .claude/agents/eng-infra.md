...
6.8. **Python 运行时 pack（dev-board#529，规格 `docs/superpowers/specs/2026-09-09-installer-slimming-design.md` §3，规范 `docs/NATIVE_PACK_DISTRIBUTION.md` §7.5）**：
   ...
   ③b **pptx 与 mineru 两个 pack 是主辅链，不是并列可替换（dev-board#615，2026-09-13 盘点）**：mineru-runtime 只跑 MinerU 官方 `fast_api` 的 `/file_parse`，**Java 侧零直接调用**，全部经 pptx-service 的 `/api/pdf/ocr-markdown` 转发（`file_parser_service.py`）；带文字层 PDF 转 Word 走 pptx-service 的 `pdf2docx`，与 mineru 无关；扫描件两个都要装。「云端 MinerU 兜底」在发行版里不可达：桌面端从不注入 `MINERU_TOKEN`，pptx-service 设置页被 `PPTX_SETTINGS_TOKEN` 恒 403。`pptx_export_editable`（可编辑导出 beta）在 `image_editability/factories.py` 无 token 必抛，Java 侧静默降级纯图片导出——**从未成功过**，砍或打通待维护者拍板。组件显示名：「PPT 生成与 PDF 转 Word」/「扫描件 OCR 引擎（MinerU）」。
