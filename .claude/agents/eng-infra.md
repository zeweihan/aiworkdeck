...
3. 产物：mac 仅 dmg（**arm64 only**，已放弃 Intel）；win 仅 nsis exe（x64）。electron-builder 配置在 desktop/package.json "build" 字段（appId com.aiworkdeck.desktop、extraResources 打入 frontend/dist、backend.jar、jre、python、pysvc.tar.gz+meta、**graphviz、skills（随包内置 skill，v0.11.1 以前漏打）、litviz（诉讼可视化引擎）**；notarize teamId X9B97KVA84；entitlements desktop/build/entitlements.mac.plist）。**win 侧 `nsis` 字段（安装器 UI 重设计新增）**：`oneClick:false` + `allowToChangeInstallationDirectory:true` 把默认一键装换成品牌化 assisted 向导；`installerSidebar`/`installerHeader` 指向入库的 `build/win/installerSidebar.bmp`、`installerHeader.bmp`；`include: build/installer.nsh` 只加回 electron-builder 默认不出的 Welcome 页（MUI2 的侧栏大图只挂在 Welcome/Finish 两页，不加 Welcome 页侧栏图只在最后一页闪现一次），文案全部走 NSIS/MUI 官方多语言字符串，`installer.nsh` 里刻意不硬编码任何语言。
