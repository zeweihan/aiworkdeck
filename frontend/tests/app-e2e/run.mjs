...
try {
  ...
  if (!J11_JAR) {
    ...
    //
    // 拦下来这一步的判据由 utils/submitGuide.js 的 submitNeedsGuide 现读一次
    // /version/status 得出（remoteAhead 仍取页面手上的 cloudStatus 快照）。**不能只靠
    // 页面那份 working**：它不会因为律师刚才的编辑而刷新，只欠结束工作、同事又没交
    // 新稿时清单会整个不出，而那一路后端也不报错（主线一动没动，git 回 UP_TO_DATE 算
    // 推成功，界面弹一句「已交稿」却什么都没交）——dev-board 0.44.1 清单 B3 就是这条。
    // 这一步同时钉住拦截生效 + 弹窗开窗后的自刷新生效（页面那份 working 还是旧的 false）。
    //
