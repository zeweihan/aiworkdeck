...
// Two touching deletions Writer does not group (different authors): hidden in
// the review gutter they collapse to one body position between 前 and 后.
export const adjacent = ['第一处删除', '第二处删除']
export async function adjacentDeletions() {
  const zip = packageWith()
  zip.file('word/document.xml', `<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p>${run('前')}${change('del', 1, '甲审阅人', adjacent[0])}${change('del', 2, '乙审阅人', adjacent[1])}${run('后')}</w:p><w:sectPr><w:pgSz w:w="11906" w:h="16838"/><w:pgMar w:top="1440" w:right="1440" w:bottom="1440" w:left="1440"/></w:sectPr></w:body></w:document>`)
