...
function environment({ width = 280, fails = false } = {}) {
  ...
  const realm = vm.createContext({ ctrl: controller, docSeq: 4,
    ...
    xModel: { getUndoManager: () => ({}) },
    undoStep: (_um, kind, run) => { inline.push('step ' + kind); run() },
  })
...
test("Writer's own undo/redo go through the worker's undo step; other commands do not", () => {
  ...
  e.query('.uno:Bold').dispatch({ Complete: '.uno:Bold' }, [])
  assert.deepEqual(e.inline, ['step undo', 'dispatch .uno:Undo', 'step redo', 'dispatch .uno:Redo', 'dispatch .uno:Bold'])
})
