// SPDX-FileCopyrightText: 2026 北京京微资易科技有限公司 and AI WorkDeck contributors
// SPDX-License-Identifier: AGPL-3.0-or-later
// 文档编辑器域：LibreOfficeEditor / ReviewPanel / DocDiffViewer /
// CompareDocDialog / VariablePanel / DrawioEditor 的用户可见文案。
export default {
  saveBeforeLeaving: '仍有文档未保存，请重试保存，或先关闭该文档处理未保存的更改。',
  retrySave: '重试保存',
  saveTimeout: '保存超时，请检查网络后重试。',
  saveCancelled: '保存已取消。',
  unsavedCloseTitle: '文档尚未保存完成',
  unsavedCloseBody: '保存未能完成。你可以继续编辑并重试保存，或放弃尚未保存的更改并关闭。已保存到磁盘的内容会保留。',
  discardAndClose: '放弃并关闭',
  moveTabSaveFailed: '保存尚未完成，标签暂不移动到另一侧',
  keepEditing: '继续编辑',
  // 原生「导出为 PDF」入口（dev-board#886）
  pdfExporting: '正在导出 PDF…',
  pdfExported: '已导出 PDF：{name}',
  pdfExportFailed: 'PDF 导出失败：{reason}',
  // 文件已被外部移动 / 改名、重试保存落不下去时的出口（BUG-14）
  saveCopyAs: '另存为…',
  savedCopyAs: '已另存：{name}（项目里的这份仍未更新）',
  saveCopyAsFailed: '另存失败：{reason}',

  status: {
    booting: '启动中…',
    unsupported: '当前环境不支持文档编辑',
    loadingDoc: '加载文档中…',
    initFailed: '初始化失败',
    bootFailed: '文档引擎启动失败',
    loadFailed: '文档加载失败',
    // 装载失败三分（dev-board#539）：404 = 文件已不在磁盘上（重试没有意义），
    // 下载超时/网络错 = 检查网络后重试有意义，其余沿用 loadFailed。
    fileMissingFailed: '文件已不在磁盘上',
    downloadFailed: '下载失败，请检查网络',
    ready: '就绪',
    reloading: '重新加载中…',
    reloadFailed: '重新加载失败，内容已过期',
    saving: '保存中…',
    saveFailed: '保存失败',
    movedSaveFailed: '文件已被移动或改名，改动尚未保存',
    desktopOnly: '仅桌面版可用',
  },
  boot: {
    engineStarting: '正在启动文档引擎',
    cjkFonts: '正在加载中文字体',
    layoutEngine: '正在初始化排版引擎',
    docService: '正在启动文档服务',
    openingDoc: '正在打开文档',
    almostReady: '即将就绪',
    rendering: '正在渲染文档',
  },
  reviewBtn: '审阅',
  previewReadableHint: '已可阅读，编辑器就绪后可直接编辑',
  preparingEditor: '正在准备编辑器',
  dlProgress: '文档内容 {loaded} / {total}',
  dlLoadedOnly: '已下载文档内容 {size}',
  firstOpenHint: '首次打开需初始化文档引擎，大文档会稍慢，请稍候',
  retryLoad: '一直卡着？点此重试',
  bootFailedHint: '文档引擎启动失败：{reason}。可能是网络或公司防火墙拦截了引擎资源，请检查网络后重试。',
  review: {
    revTab: '修订',
    cmtTab: '批注',
    evidenceTab: '底稿',
    checkTab: 'AI 审校',
    collapse: '收起审阅面板',
    acceptAll: '全部接受',
    rejectAll: '全部拒绝',
    bucket_ai: 'AI',
    bucket_me: '我',
    bucket_other: '其他人',
    bulkAcceptTitle: '只接受「{bucket}」的修订？',
    bulkRejectTitle: '只拒绝「{bucket}」的修订？',
    bulkAcceptBody: '将接受当前筛选「{bucket}」下的 {count} 条修订；其余 {rest} 条修订不受影响。',
    bulkRejectBody: '将拒绝当前筛选「{bucket}」下的 {count} 条修订；其余 {rest} 条修订不受影响。',
    emptyRevTitle: '没有待处理的修订',
    emptyRevSub: '对文档的改动会以修订形式出现在这里，可逐条接受或拒绝',
    loading: '正在读取…',
    deletion: '删除',
    insertion: '插入',
    table: '表格',
    contiguousCount: '{count} 处连续',
    unknownAuthor: '未署名',
    emptyText: '（空）',
    emptyCmtTitle: '没有批注',
    emptyCmtSub: 'AI 的说明与提醒会挂成批注，不会写进正文',
    resolved: '已解决',
    anchor: '锚定：{text}',
    reopen: '重新打开',
    resolve: '标记解决',
    reply: '回复',
    replyPlaceholder: '回复这条批注',
    replySend: '发送',
    replyCancel: '取消',
    accept: '接受',
    reject: '拒绝',
    opFailed: '操作未成功',
    limitReached: '此列表最多显示 {count} 条，超出部分暂未加载。',
    groupPartialFail: '本组 {total} 处中有 {failed} 处未被引擎处置',
    typeFormat: '格式',
    typeParaFormat: '段落格式',
    typeOther: '其他修订',
    filterAll: '全部 {count}',
    filterAi: 'AI {count}',
    filterMe: '我 {count}',
    filterOther: '其他人 {count}',
    selfAuthor: '{name}（我）',
    emptyFilteredTitle: '这一类没有修订',
    emptyFilteredSub: '换一个筛选条件，或点「全部」看所有作者的改动',
    reasonBy: '{author} 的修订理由',
    linkedToRev: '已挂到 {count} 条修订',
  },
  diff: {
    title: '文档对比',
    prev: '上一处',
    next: '下一处',
    sideBySide: '并排',
    inline: '内联',
    sourceLabel: '源文档: {name}',
    targetLabel: '新文档: {name}',
    loadingContent: '正在加载文档内容...',
    retry: '重试',
    sourceDocDefault: '源文档',
    newDocDefault: '新文档',
    prevVersion: '上一版',
    thisVersion: '这一版',
    fetchVersionFailed: '获取版本文件内容失败',
    fetchDocFailed: '获取文档内容失败',
    loadFailed: '加载文档失败',
    containerMissing: '找不到编辑器容器',
    initFailed: '初始化编辑器失败: {msg}',
    monacoLoadFailed: '加载 Monaco Editor 失败',
  },
  compare: {
    title: '选择对比文档',
    desc: '请选择源文档（作为对比基准）和新文档（与源文档比较差异）',
    sourceLabel: '源文档（基准）',
    arrowLabel: '对比',
    targetLabel: '新文档（比较对象）',
    cancel: '取消',
    start: '开始对比',
  },
  vars: {
    loading: '加载中...',
    empty: '暂无变量',
    noMatch: '未找到匹配的变量',
    insert: '插入',
    updateValue: '更新值',
    delete: '删除',
    confirmDelete: '确认删除?',
    cancel: '取消',
    confirm: '确定',
    emptyValue: '（空）',
    docScope: '文本变量',
    projectScope: '项目变量',
    userScope: '用户变量',
    modalTitle: '将选中文字设为变量',
    modalSubtitle: '变量名建议简短且可复用（同名会覆盖）',
    namePlaceholder: '请输入变量名，例如：主营业务',
    create: '创建',
    activateEditorFirst: '请先点击激活一个编辑窗口',
    selectInDocFirst: '请先在文档中选择内容',
    noFieldInsert: '当前 WPS 组件未提供“域插入”能力',
    created: '已创建',
    createFailed: '创建失败',
    inserted: '插入成功',
    insertFailed: '插入失败',
    selectContentFirst: '请先选择内容',
    confirmUpdateTitle: '确认更新',
    confirmUpdateContent: '确认将变量 "{name}" 更新为选中文本？',
    updated: '更新成功',
    updateFailed: '更新失败',
    syncing: '同步中...',
    syncDone: '同步完成 ({count})',
    syncFailed: '同步失败',
    deleted: '已删除',
    deleteFailed: '删除失败',
  },
  drawio: {
    opening: '正在打开图形编辑器…',
    unavailable: '当前环境没有内置图形编辑器',
    downloadHint: '这份 {name} 可以下载后用 draw.io 打开继续编辑。',
    downloadFile: '下载文件',
    retry: '重试',
    unsaved: '未保存',
    saving: '保存中…',
    saved: '已保存',
    openFailed: '打开失败',
    fileMissing: '文件不存在',
    readFailed: '读取文件失败（{status}）',
    fileEmpty: '文件是空的',
    saveFailed: '保存失败',
    installHint: '图形编辑器组件还没装，装好后可以继续编辑这份图。',
    installPack: '安装图形编辑器组件',
    installPackDownloading: '正在安装…',
    installPackProgress: '正在下载… {downloaded} / {total} MB',
    installPackRetry: '安装失败，重试',
    installPackFailed: '安装失败',
  },
  // 纯文本轻量编辑器（PlainTextEditor.vue，dev-board#37）
  plainText: {
    opening: '正在打开文本编辑器…',
    loadFailed: '读取文件失败',
    readFailed: '读取文件失败（{status}）',
    fileMissing: '文件不存在',
    emptyDownload: '文件内容下载为空，为防覆盖真实内容已禁止编辑，请重试',
    retry: '重试',
    edit: '编辑',
    preview: '预览',
    unsaved: '未保存',
    saveFailedRetry: '保存失败，点击重试',
  },
  // 自建工具栏（EditorToolbar.vue）。工具提示、下拉与查找替换条的全部可见文案。
  // AI 审校（dev-board#723/#724，命名与开关归属见 #749）：清单在审阅面板的
  // 「AI 审校」页，正文里只剩一颗浮球——开关也在那颗浮球上。
  inlineReview: {
    local: '即时规则检查',
    checking: '正在检查',
    stale: '正文已变化，等待重新检查',
    error: '检查暂不可用',
    offTitle: 'AI 审校已关闭',
    offHint: '开启后会在你停笔约 20 秒时自动跑一遍 AI 审校（按用量计费）。结构、编号、交叉引用、算式与证件号码的规则检查不受影响，始终在跑。',
    autoPaused: '自动 AI 审校本次已暂停，可用上方按钮手动重试。',
    empty: '此分类暂无问题',
    staleItem: '等待重新检查',
    locate: '定位原文',
    apply: '采用建议',
    ignore: '忽略本条',
    refresh: '重新检查',
    deep: '立即 AI 审校',
    deepBusy: 'AI 审校中…',
    deepRetried: '（已自动重试一次仍失败）',
    aiEnable: '开启 AI 审校',
    aiDisable: '关闭 AI 审校',
    collapseBall: '浮球收起到边缘',
    expandBall: '展开正文浮球',
    scope: '当前检查范围：正文段落；表格、页眉页脚等请使用完整核验。',
    truncated: '本次检查有截断，不能视为全文检查完成。',
    applied: '已采用建议，可撤销。',
    failed: '正文或版本已变化，请重新检查。',
    bucket: {
      all: '全部',
      supplement: '待补充',
      consistency: '一致性',
      format: '格式与号码',
      ai: 'AI 审校',
    },
    err: {
      REVIEW_INLINE_REVISIONS: '请切换到页边修订或最终视图后再审校。',
      REVIEW_SNAPSHOT_FAILED: '暂时无法读取当前文档，请重新检查。',
      REVIEW_FAILED: '本次审校未完成，请重新检查。',
      REVIEW_DEEP_INCOMPLETE: 'AI 审校未完整完成，以下仅为已完成的检查，可重试。',
    },
    deepReason: {
      DEEP_TIMEOUT: '原因：AI 模型没有在时限内返回。',
      DEEP_BUDGET: '原因：本次 AI 审校时间用尽，正文后面的部分还没检查。',
      DEEP_UNPARSEABLE: '原因：模型返回的内容无法解析，可能被截断。',
      DEEP_UPSTREAM: '原因：模型服务暂时故障。',
      DEEP_NETWORK: '原因：本机连不上模型服务，请检查网络或代理。',
      DEEP_RATE_LIMITED: '原因：模型服务限流，请等一分钟再试。',
      DEEP_QUOTA: '原因：账户额度不足，请先充值或分配额度。',
      DEEP_TOO_LONG: '原因：这一段正文超出模型上下文，请分段后再审校。',
      DEEP_MODEL_UNAVAILABLE: '原因：当前辅助模型不可用，请到设置页更换。',
      DEEP_REGION: '原因：当前网络环境被服务商按地域拒绝，请更换模型或网络。',
      DEEP_FAILED: '原因：模型调用失败。',
    },
  },
  toolbar: {
    undo: '撤销 (Cmd/Ctrl+Z)',
    redo: '重做 (Cmd/Ctrl+Shift+Z)',
    paraStyle: '段落样式',
    font: '字体',
    fontFallback: '字体',
    fontSize: '字号',
    bold: '加粗 (Cmd/Ctrl+B)',
    italic: '倾斜 (Cmd/Ctrl+I)',
    underline: '下划线 (Cmd/Ctrl+U)',
    strikeout: '删除线',
    superscript: '上标',
    subscript: '下标',
    textColor: '字体颜色',
    highlight: '突出显示',
    clearFormatting: '清除格式',
    alignLeft: '左对齐',
    alignCenter: '居中',
    alignRight: '右对齐',
    alignJustify: '两端对齐',
    bulletList: '项目符号',
    numberList: '编号列表',
    indentLess: '减少缩进',
    indentMore: '增加缩进',
    formattingMarks: '显示格式标记',
    trackChanges: '记录修订',
    trackChangesShort: '修订',
    revisionView: '修订显示方式',
    revisionViewAll: '正文内显示修订',
    revisionViewMargin: '批注框中显示修订',
    revisionViewFinal: '最终稿',
    reviewPanel: '审阅面板',
    reviewShort: '审阅',
    semanticWriting: '有据续写（结合项目材料生成建议，点「采用」才写入正文）',
    semanticWritingShort: '有据续写',
    autocomplete: '自动补全（本地词库，打字时给候选；不调模型、不联网）',
    autocompleteShort: '自动补全',
    insightPanel: '解析本文并打开「依据」窗格',
    insightShort: '解析',
    zoom: '显示比例',
    insert: '插入',
    insertTable: '表格…',
    insertImage: '图片…',
    insertLink: '超链接…',
    insertComment: '批注…',
    insertPageBreak: '分页符',
    needSelection: '需先选中文字',
    gridPrompt: '选择行列数',
    gridSize: '{rows} × {cols} 表格',
    linkTitle: '给「{sel}」加链接',
    commentTitle: '对「{sel}」批注',
    commentTitleNoSel: '添加批注',
    linkPlaceholder: 'https://',
    commentPlaceholder: '批注内容',
    cancel: '取消',
    confirm: '确定',
    linkRequired: '请填写链接地址',
    commentRequired: '请填写批注内容',
    opFailed: '操作未成功',
    imageReadFailed: '图片读取失败',
    find: '查找',
    findAndReplace: '查找和替换',
    findPlaceholder: '查找',
    replacePlaceholder: '替换为',
    prevMatch: '上一个',
    nextMatch: '下一个',
    replace: '替换',
    replaceAll: '全部替换',
    matchCase: '区分大小写',
    close: '关闭',
    noMatch: '无匹配',
    matchPosition: '{index} / {total}',
    matchTotal: '共 {total} 处',
    findFailed: '查找失败',
    replaceFailed: '替换失败',
    nothingReplaced: '没有找到可替换的内容',
    // 表格上下文组：光标在表格里时才出现
    tableGroup: '表格',
    rowAbove: '在上方插入行',
    rowBelow: '在下方插入行',
    colLeft: '在左侧插入列',
    colRight: '在右侧插入列',
    deleteRow: '删除本行',
    deleteCol: '删除本列',
    insertFootnote: '脚注…',
    editHeader: '页眉…',
    editFooter: '页脚…',
    footnotePlaceholder: '脚注内容',
    headerPlaceholder: '页眉文字',
    footerPlaceholder: '页脚文字',
    footnoteTitle: '在光标处插入脚注',
    headerTitle: '设置页眉',
    footerTitle: '设置页脚',
    textRequired: '请填写内容',
    nativeMenus: '原生菜单',
    nativeMenusTitle: '临时显示 LibreOffice 原生菜单栏与工具栏',
    // 段落样式的中文名。引擎给的 DisplayName 在部分构建上回英文，这张表兜底。
    styleNames: {
      'Standard': '正文',
      'Default Paragraph Style': '正文',
      'Text body': '正文文本',
      'Heading': '标题',
      'Heading 1': '标题 1',
      'Heading 2': '标题 2',
      'Heading 3': '标题 3',
      'Heading 4': '标题 4',
      'Heading 5': '标题 5',
      'Heading 6': '标题 6',
      'Title': '文档标题',
      'Subtitle': '副标题',
      'Quotations': '引用',
      'List': '列表',
      'Caption': '题注',
      'First line indent': '首行缩进',
      // BUG-36：这三个引擎内置样式名（program name，跨语言固定）原来没有
      // 走本地化表，退到 office_thread.js 的 DisplayName——那份显示名在同一
      // 会话内新建样式定义时可能沿用还没绑定完 UI locale 的取值，重开文档才对，
      // 表现就是英文「Table Contents」。补进表后不再依赖引擎那次绑定时机。
      'Table Contents': '表格内容',
      'Table Heading': '表格标题',
      'Contents Heading': '目录标题',
    },
    colors: {
      auto: '自动', red: '红', ink: '墨绿', blue: '蓝', brown: '棕',
      purple: '紫', darkGray: '深灰', gray: '灰',
      none: '无', yellow: '黄', green: '绿', cyan: '青', magenta: '品红',
    },
  },
}
