...
export default {
  ...
  decisionAssistOnHint: '尝试减少工具说明 token 和总费用；会多一次「工具预选」判断，产生少量额外费用，回复可能稍慢。',
  decisionAssistOffHint: '关闭时不做工具预选，不产生额外请求或费用。开启后尝试省 token 和总费用，回复可能稍慢。',
  decisionAssistNextSend: '对下次发送生效；已提交的任务保持原设置。',
  decisionAssistDetails: '数据与费用说明',
  decisionAssistData: '开启后，本次输入与可用工具类目、工具名会经当前 OpenRouter 通道发送至第三方工具预选服务 TypeSafe Jev，按平台额度或自带密钥计费。',
  decisionAssistMaterials: '不会为工具预选额外附带附件、历史会话或文档正文；你粘贴或写在本次输入里的内容仍会发送。',
  decisionAssistFallback: '只辅助预选工具类目，不更换你选定的主模型。单次判断最多等待 2.5 秒；低置信度、超时或不可用时按原流程处理。',
  decisionAssistLocal: '当前使用本地模型，不做工具预选，不外发内容，也不产生额外费用。',
  ...
  // 价格（dev-board#853）：每行只有两个数，单位、口径、档位含义统一写在下拉底部脚注
  modelPriceUnitCny: '单位：元 / 百万 tokens，前为输入、后为输出',
  modelPriceUnitUsd: '单位：美元 / 百万 tokens，前为输入、后为输出',
  modelPriceChargedCny: '实付价：厂商美元标价 × 账户扣费汇率 {rate}',
  modelPriceChargedUsd: '实付价：厂商美元标价 × 账户扣费系数 {rate}',
  modelPriceChargedSplitCny: '实付价 = 厂商美元标价 × 汇率 {rate} × 平台系数 {margin}',
  modelPriceChargedSplitUsd: '实付价 = 厂商美元标价 × 平台系数 {margin}',
  modelPriceWithMeta: '{main}（{meta}）',
  modelPriceMetaSep: '，',
  modelPriceSourceLive: '即时汇率',
  modelPriceSourceManual: '人工设定汇率',
  modelPriceSourceDefault: '默认汇率',
  modelPriceUpdatedOn: '更新于 {date}',
  modelPriceListPlatform: '厂商美元标价，实际按 Credits 扣费（暂未取到账户扣费汇率）',
  modelPriceListByok: '厂商美元标价：这台机器仍在用旧的自备 Key 设置，由你自己的 OpenRouter 账户直接付费；切回 AI WorkDeck 官方通道后按 Credits 扣费',
  modelPriceListLocal: '厂商美元标价，仅供参考；本地模型不计费',
  modelPriceListUnknown: '厂商美元标价',
  modelPriceLevelLegend: '颜色越深越贵：经济、标准、高端、旗舰四档（按 80% 输入 + 20% 输出的综合单价分档）',
  modelPriceLevel1: '经济',
  modelPriceLevel2: '标准',
  modelPriceLevel3: '高端',
  modelPriceLevel4: '旗舰',
  modelPriceLevelTitle: '价格档位：{name}',
  modelPriceAria: '输入 {input}，输出 {output}，每百万 tokens',
  ...
  carriedAttachmentHint: '虚线标签是上轮附件，下一条继续使用，点 × 移除',
