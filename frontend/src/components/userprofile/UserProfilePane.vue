<!--
  ...
  从 pages/userprofile/userprofile.vue 整体抽出来，为的是**同一份内容有两个宿主**（照
  components/admin/AdminPane.vue + pages/admin/admin.vue 的先例）：
   · 工作台中栏的「个人中心」标签（主用法，顶栏右上角头像下拉进来）；
   · pages/userprofile 薄壳页（直链、浏览器端、以及仓里既有的
     navigateTo '/pages/userprofile/userprofile'，一条都没断——那条 navigateTo
     依赖页面栈保留工作台实例以便 onShow 回流刷新，check-navigation-contract.mjs 有断言守着）。

  页面生命周期换成组件生命周期，别的一律没动：
    onLoad()   → mounted()
    （原文件没有 onUnload，新增 beforeUnmount() 只做一件事：清掉两个绑定验证码的
    倒计时定时器——标签常驻工作台里不会像整页那样随导航销毁重建，定时器不清会跨标签泄漏）

  两处必须处理的副作用（embedded 时行为要变）：
   (1) mounted 里的 host.browser.setViewsVisible({visible:false}) 只在非 embedded 时执行——
       工作台自有 BrowserView 显隐管理（desktopOverlayActive watcher），照抄会把用户开着的
       浏览器标签隐藏掉。
   (2) 「返回项目列表」出口只在非 embedded 时渲染——标签靠自身的关闭按钮退出，
       embedded 场景下没有页面栈可退。
-->
<template>
  <view class="page-userprofile" :class="{ 'is-embedded': embedded }">
      ...
      <view class="user-sidebar">
        ...
        <view class="user-card">
          ...
          <!-- 返回项目列表：出口，不是 tab。故意放在 .nav-menu 外面而不是它的第一个子节点——
               ...
               embedded（工作台标签）时不渲染：标签靠自身关闭按钮退出，没有页面栈可退。 -->
...
export default {
  ...
  mounted() {
    // Desktop：非 embedded（直链薄壳页）时必须隐藏 BrowserView（避免工作区网页残留覆盖）。
    // embedded（工作台标签）时工作台自有 desktopOverlayActive watcher 管 BrowserView 显隐，
    // 这里再调一次会把用户开着的浏览器标签隐藏掉。
    if (!this.embedded) {
      try {
  ...
  beforeUnmount() {
    // 原页面没有 onUnload，这里新增只为清两个倒计时定时器：标签常驻工作台里
    // 不会像整页那样随导航销毁重建，定时器不清会跨标签泄漏。
...
/* 嵌在工作台中栏的标签里：铺满标签内容区并自己滚动。
   ...
   写 min-height:100vh 会让整块比容器高，把滚动条推给外层（照 AdminPane.is-embedded 先例）。 */
.page-userprofile.is-embedded {
  min-height: 0;
  height: 100%;
  overflow-y: auto;
  padding: 20px 16px;
...
.page-userprofile.is-embedded .sidebar-logo-area {
  display: none;
}
