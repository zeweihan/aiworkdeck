...
桌面端首次启动会要求填一个解锁码。没有注册、没有登录：解锁码在本机填一次，之后每次打开都直接进入工作台。
...
试用码由应用内置的公钥离线验签，解锁过程不联网，也不会把本机的任何信息发往外部。试用版全部功能均可正常使用，只是工作台顶栏常驻一枚「试用版」标识。

**正式版**：在[官网](https://www.aiworkdeck.com)注册账户，在账户页生成账户 Key（`awdk_` 前缀），粘进同一个解锁输入框即可。账户 Key 走在线校验，解锁为正式版的同时把桌面端与您的账户连上——已购内容随之同步，平台 AI 通道也一并可用。随时可在设置页「账户与用量」中解除连接。

两点供拟部署的机构参考：解锁是按机器的，状态存在本机 `~/.aiworkdeck/` 目录下；部署为团队服务器（同事用浏览器访问共用服务端）时不设解锁门。
...
| 语音合成 | 本地引擎（随包 Kokoro） | 可以 | 默认在本机合成；可改自备 Key 走 ElevenLabs |
...
**平台代采档会经过我们的服务器。** 桌面版全新安装时，图片文字识别、联网搜索、企业工商信息、证券财务数据、法律法规检索与会议录音转写默认落在「平台代采」档——由 AI Workdeck 统一向供应商采购、按 Credits 计费，不必自备 Key；该档下本次调用必需的内容会经我们的服务器转给对应供应商。其中只有会议录音的音频文件会在我们的对象存储中转，转写完成即删除，另有 24 小时生命周期规则兜底；其余各项只在调用当时透传，不留存请求内容。语音合成默认用随包的本地引擎，不出本机。逐项口径（经过什么、存放多久、何时删除）见 [`legal/PRIVACY.md`](legal/PRIVACY.md)。
