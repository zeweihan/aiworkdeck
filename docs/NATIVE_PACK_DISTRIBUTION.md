...
6. 失败处置：验签/哈希失败删除对应产物；**网络中断保留 `.part`**（下次续传）；
   ...
   不保留 `.part`**：`.part` 已经跟服务器的认知对不上了（典型触发：本地 `.part`
   达到或超过 manifest 声明的组件 `size`，下一次续传请求的 Range 起点必然越界），
   带着它重试只会一直收到同一个 4xx——收到即删 `.part`，同一轮 `MAX_ATTEMPTS`
   循环内下一次尝试就会发全量请求自愈，不必等用户再点一次「安装」。发起请求前也有
   一道构造性防御：`.part` 大小已不小于声明 `size` 时提前清空，尽量不让越界 Range
   真的发出去（2026-08-20 修复的死循环：旧实现把 416 当网络抖动处理，`.part` 越
   留越挨打，用户重试/卸载都打不破循环）。
...
- 卸载是幂等的收口动作：`packs/<id>/` 目录不存在（从未装成功过，比如安装反复
  416 失败）时**不抛异常**，只清理 `.staging/<id>-<version>/` 残留后正常返回。
  旧实现在这种情况下抛「资源包未安装」，配合 §4.2 的 416 死循环会把用户卡在
  「装不上、卸不掉」两头堵；卸载端点不该比安装端点更挑剔。
