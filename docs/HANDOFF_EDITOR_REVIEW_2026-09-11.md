...
> **状态更新（2026-09-11 夜，Claude 接续完成，随 v0.41.0 发布）**：下文是 Codex 暂停时的原始交接记录，保留作历史。
> 其后的处理：#600 IME 空输入/删除边界与回显去重已修；#601 改由 worker 的 `XContextMenuInterceptor`
> 在原生弹出前裁决二选一（不再合成 Escape）；新引擎上打字等审阅元数据约 1s 的根因已消除（位置读原生几何、
> 元数据按原生身份缓存、停笔后重读）；页边/气泡视图撤销卡死改为「在记录它的视图里撤销」；独立复核的 15 条
> 缺陷（ReviewPanel 围栏 Proxy、按原生 id 处置修订、外部审阅区越界读等）已修；r5 引擎补上 Impress 并焙入
> 越界修复后重新编译发布为 `24.2.8-zhcn-r5`。长期契约以 `.claude/agents/doc-editor.md` 为准。

2026-09-11，用户因 Codex 额度不足，明确要求暂停、按现状提交，交由 Claude 继续。**这是 WIP 快照，不可合并发布；没有完成验收。** 不要把“已编译”“旧引擎测试通过”当成新引擎或用户体验通过。

最终保存状态：代码快照 `dfcc8ef5efe139d0a283878a3df2b034ab087268` 已推送到上述分支（之后仅补充本交接记录）。四文件已取回并通过 SHA-256、WASM 魔数与 metadata 大小检查；新 wasm SHA-256 为 `3b52e79c1e2fe971626996565099852af873cead4f2c83aac47325d5bb9db512`。构建机已于 **06:12:21 UTC / 14:12:21 UTC+8** 主动释放，API 已确认不存在；临时 OSS 私有传输对象已删除。全过程未发布新版本。
