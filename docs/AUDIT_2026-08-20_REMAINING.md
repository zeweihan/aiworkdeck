...
|---|---|
| 已修并合并 | 156 |
| 复现后被推翻（不是缺陷 / 不可达 / 上一轮已修漏剔） | 6 |
| 留给维护者拍板 | 4 |

做法自始至终一致：每条自己读代码走完链路 → 写能复现的测试 → **先看它红** →
改实现看它绿 → **把实现还原确认测试真会红**（防空断言）。判不准的不改，写明理由。

### 修复的 PR（按合并顺序）

第一轮：#500 #501 #503 #504 #507 #508 #509 #510 #512 #513 #514 #515 #516 #517
第二轮：#519 #520 #521 #522 #523 #524 #525 #526

覆盖范围逐条写在各 PR 的正文里，包括每条的病灶、修法、红/绿证据，
以及自定阈值（那些不是产品拍板的数字，都在 commit 里标注了是谁定的）。

### 复现后被推翻的 6 条（别再当成待办）

- **getFileBytes 返回模板字节**：上一轮 PR #487 已修，`LocalFileStorageService.load` 现在是纯读。清单漏剔。
- **移出客户不作废访问码**：前端 `clientLogin` 恒传 `displayName=null`，被移出的就是 `relatedUserId` 那个用户，作废是生效的。只有直接调 API 且传 displayName 那条路会漏，仓内没有任何客户端那样调。
- **attachMaterial 并发丢更新**：前端一次只处理一个文件且 await 完再刷新，找不到能同时打同一个列表槽的第二个调用方。
- **renderDocx / renderPptx 缺竞态守卫**（2 条）：桌面壳 `libreOfficePreferred` 恒真、规范 Web 部署也带引擎探针，docx/pptx 全部被截给 LibreOffice 编辑器，这两段代码只有 dev server 或漏部署引擎的站点才会执行。
- **邀请对话框按钮无重入守卫**：后端有唯一约束且通用码命中已有邀请就直接返回原码，天然幂等，产生不了重复成员或重复访问码。
...
1. **`pdf_redact` 部分命中现在算成功**并如实报告缺失项（原来是抛异常，但磁盘已经被改过了，
   ...
   若口径相反，改法是先在临时文件里完成全部处理、确认无 missing 才原子替换原文件。
2. **OCR 屏幕共享改成离开页面才释放**，保留了「授权一次后同页内反复截图不再弹系统选择器」
   的原设计。若期望是「关闭浮层即释放」，把 beforeUnmount 里那几行挪进 closeOcrOverlay 即可。

### 两个仍然存在的已知局限（修的时候如实写下来的，不是遗漏）

- **PDF 脱敏整页累积匹配**：相邻两个无关数字串会被拼成一个长数字串。#521 给
  ID_CARD / BANK_CARD 加了校验位判定，把这类误判压掉了绝大部分；没有校验位的模式仍有理论风险。
- **`OptimizerMailer` 的收件去重记在进程内存里**：覆盖同进程内的重试，进程重启后紧邻的
  下一轮仍可能重发一次。做成跨重启需要新表或加列。
