...
> **2026-08-21 更新：本清单里约 64 条已修完并合并（PR #500/#501/#503/#504/#507/#508/#509/#510/#512/#513/#514/#515/#516）。**
> 下面「本轮已修」一节列了逐 PR 的覆盖范围与刻意没做的部分；再往下是**原始清单原文**，
> 其中已修条目未逐条删除（删了就没法回查当时的判断），请以「本轮已修」为准。

## 本轮已修（2026-08-21）

做法与上一轮相同：每一条自己读代码走完链路 → 写能复现的测试 → **先看它红** →
改实现看它绿 → **把实现还原确认测试真会红**（防空断言）。判不准的不改，写在下面。

| PR | 覆盖 |
|---|---|
| #500 | 验证码签发/核销并发丢更新；pack 封禁位被幂等重装抹掉；SSE 手工 JSON 漏控制字符；工具参数被 `({...})` 顶掉；法律信息保护合并重叠片段吃掉金额单位；查找替换 no-op 被报成「已修改」；TTS 读超时误报「组件未下载」；单文件 move 补缓存区额度闸（纵深） |
| #501 | 通用访问码登录后没有任何项目权限（清单外，复核时发现）；邀请链接被并发签发一次即永久 500；反馈上传把「云端拒收」读成「已收下」 |
| #503 | 网页代理无体积上限/无总时限；`/api/cloud/*` 缺字段直接 NPE/ClassCastException；编辑器结果回传的鉴权键错配（纵深） |
| #504 | Office 插件 localStorage 白屏；引擎资产截断下载过校验；端口链断言恒真；matrix 发半成品 Release（含审校补的 `sync-mirror` 顺序）；`npm ci \|\| npm install` 后门；启动脚本四条 |
| #507 | 上下文压缩丢掉最后一段正文；一个 `tool_code` 块里的第二条调用被静默吞掉 |
| #508 | 空 body 上传把已有文件截成 0 字节；对账丢 truncated 标志；内层事务抛异常毒化整轮（含审校补的 `openLocalFolder` 同形状）；尽调上传超长文件名留孤儿对象；巨潮抓取留孤儿空文件行 |
| #509 | PDF 脱敏漏掉被拆开的敏感串（含审校补的按行分框）；DOCX 脱敏从不扫页眉/页脚/脚注/尾注 |
| #510 | 回收站还原后文件永久消失；回收站可拖拽改坏 parentId；批量彻底删除吞失败报全体成功；OCR 屏幕共享从不释放；录音设备中断仍显示「录音中」；删收藏刷新被节流吞掉；TOTP 慢响应覆盖新密钥；TTS 卸载后自动播放泄漏；.drawio 两次保存有一次静默烂尾 |
| #512 | 股东大会材料查找忽略软删除；剪贴板删记录不删底层对象；任务标题类型错误报「服务器内部错误」 |
| #513 | 项目记忆关键日期写一次就冻住；同项目并发记忆提取互相覆盖 |
| #514 | 插件工具缓存从不失效（更新后仍跑旧代码）；空消息永久毒化会话；卡住的 RUNNING 任务永不回收；文档检查点只按会话存导致恢复错文档；pptx 真实失败原因被丢弃；下载只看 200 空文件也算成功；litviz 运行时缓存装完 pack 不生效；插件 jar 缺失零日志；插件安装拷贝兜底失败留残缺目录；`pdf_redact` 写盘后才抛异常 |
| #515 | 设置页显式选了本地 Ollama，白名单模型仍把内容发去 OpenRouter（辅助模型默认 id 就在白名单里，等于每次辅助调用都走云端） |
| #516 | 现场影像标成已送达却永久丢失（CRITICAL）；clientMediaId 幂等 TOCTOU；转写结果解析失败被当成空会议；转码无超时堵死唯一转写线程；读时轮询并发双扣；历史版本文件无体积闸；陈旧 `.git/index.lock` 无人清理 |

### 复核时被推翻或收窄的（记录下来，别再当成待办）

- **getFileBytes 返回模板字节**（原 HIGH）：上一轮 PR #487 已修，`LocalFileStorageService.load` 现在是纯读。清单漏剔。
- **移出客户不作废访问码**（原 HIGH）：按当前前端（`clientLogin` 恒传 `displayName=null`），被移出的就是 `relatedUserId` 那个用户，作废是生效的。只有直接调 API 且传 displayName 的那条路会漏，仓内没有任何客户端那样调。判为不可达。
- **单文件 move 绕过缓存区额度**（原 HIGH）：`__staging_area__` 在文件树里是隐藏的、缓存区自己的拖入拖出一律走 batchMove，没有用户可达的绕过路径。仍补了闸，但按纵深防护而非可达绕过。
- **attachMaterial 并发丢更新**（原 HIGH）：前端一次只处理一个文件且 await 完再刷新，找不到能同时打同一个列表槽的第二个调用方。
- **EditorResultController 跨会话注入**（原 HIGH）：`requestId` 是 UUID、只经该会话自己的 SSE 下发、30 秒内必回收，找不到泄露面。按纵深修了，不夸大严重度。

### 刻意没做的

维护者需先拍板的三条（沿用 #498 的判断，未动）：`AgentOrchestrator` 并发轮次竞态（含「停止后立刻再发会擦掉上一轮的取消标志」，仍需 runId/RunGuard 架构改动）；`SensitiveType.CHINESE_NAME` 正则；会议永远停在「转写中」。

本轮新增的三条，都是判不准或代价大于收益：

- **`SseEmitterService.close(connectionId)` 用单参 remove**，会掐掉重连后的新 emitter。同文件其它三处都用了两参 remove 且注释点名了这个风险，缺陷确凿；但正确修法要给每轮一个 emitter 令牌，与上面那条架构改动是同一处，且前端本身带指数退避重连，证不到「用户可见故障」的程度。
- **缓存区额度 / `ensureFolder` 的 check-then-act 竞态**（3 条）：正确修法要跨事务边界（锁必须活到提交之后，或者给 `(project_id, parent_id, name)` 加唯一约束）。前者在 `@Transactional` 方法里加进程锁是假修（锁在提交前就释放了），后者在 `ddl-auto: update` + 可能已有重复数据的前提下有风险。建议做法记在这里，留给维护者决定。
- **`MatterClassifierService` 去重位在异步 AI 调用成功前置上**：属实，但 `classifyAsync` 只在会话首轮被调用，移除去重位也拿不到第二次机会；丢一条会话的事项类别是纯遥测，无用户可见影响。真正值得做的是给 `classify()` 加重试与给 `classified` 加上界。
- **`AuthController.getUsernameFromSession` 吞掉所有异常返回 null**：属实，但修法只是补一条日志，不改变任何用户可见行为。

### 需要产品口径确认的两处（本轮按判断先做了，改回都很容易）

1. **`pdf_redact` 部分命中现在算成功**并如实报告缺失项（原来是抛异常，但磁盘已经被改过了，
   调用方的 `finishModification` 被跳过，DB 与预览停在旧版本）。理由：redact 不可逆且以秒计生效，
   不该因为清单里一个查无此文的名字，让已经定位到的敏感信息也不落地。
   若口径相反（宁可全部不做也不能半吊子脱敏），改法是先在临时文件里完成全部处理、
   确认无 missing 才原子替换原文件。
2. **OCR 屏幕共享改成离开页面才释放**，保留了「授权一次后同页内反复截图不再弹系统选择器」
   的原设计。若期望是「关闭浮层即释放」，把 `beforeUnmount` 里那几行挪进 `closeOcrOverlay` 即可。

### PDF 脱敏的一个取舍（未擅自收紧）

整页累积匹配意味着相邻两个无关数字串会被拼成一个长数字串，变长数字模式
（`BANK_CARD` 是 `\d{16,19}`）理论上可能因此误命中，把两处正常数字一起涂黑。
这与 `CHINESE_NAME` 那条「过度脱敏」属同一类产品口径问题。需要收紧的话，
方向是给数字类模式加「前后不接数字」的断言。
