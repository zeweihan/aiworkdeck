...
**两种时机，判据是同步还是异步**（P0 实施时修正：初稿写「按单次估算金额分」，
但对同步调用而言「预扣 → 结算」严格劣于「预检余额 → 事后扣」——多两行流水、
多一个可能悬挂的状态，防的却是同一件事。预扣真正解决的是**异步任务的长间隔**：
提交与出结果之间隔着几十分钟，钱必须在这段时间里被占住）：

- **同步调用 → 事后扣**：先按客户端申报的计量做一次余额只读预检（让用户在我们向上游
  付费之前就被拦住），调用成功后按**上游返回的真实计量** `spendCredits` 一笔。失败不扣。
- **异步任务 → 预扣 → 结算**：见 §4.4。今天只有 `asr` 走这条。

`HOLD_THRESHOLD_CENTS` 保留，但用途收窄为**提交前是否要用户确认**：
超过这个估算金额的动作，桌面端在提交前显示「本次约需 N Credits」。
它是用户闸门，不是记账开关。
...
#### 6.2.2 修订三（P3 实施时发现，本节初稿没覆盖到的五条）

1. **Whisper 说普通话时稳定输出繁体。** 本机实测两分钟的普通话会见录音，
   内容几乎全对但整篇是繁体字（「爭議焦點」「股權轉讓」）。对大陆律师来说这是一份
   要整篇重排的笔录。社区常用的 `initial_prompt` 书写体偏置**一个字都没纠正过来**
   （Whisper 的 prompt 是「上文示例」不是「指令」）。落地改成确定性的后处理：
   OpenCC 繁转简（纯 Python，1.1MB，不出网），`ASR_OUTPUT_SCRIPT=original` 可关。

2. **速度约为实时的 1.5 倍，必须写进界面。** medium + int8 + VAD 在 M 系列 CPU 上
   实测 122 秒音频跑 78 秒，一场两小时的会要跑一小时上下。设计只说了「本地档作隐私选项」，
   没说它慢——而用户在按下录音键之前就该知道这件事，理由与 §6.2.1 完全相同。

3. **「服务没起」与「模型没下」必须是两个可观测的状态。**
   因此 `asr-service` 与 kokoro 有一处刻意的不同：kokoro 用 `enabled` 把服务门在模型上，
   asr-service **无论有没有模型都启动**。不起进程的话探测只剩「服务没起」一种结论，
   用户照提示重启一万次也不会有模型。

4. **local 档全程没有 taskId，「被关机打断」需要另一条判据。**
   云端两档靠落库的 taskId 恢复轮询；本地档整段推理在后台执行器里跑完，
   重启后库里只剩一个「转写中」，而 `startTranscription` 对该状态是幂等返回——
   用户连重试都点不动。加了进程内的 `inFlight` 集合：两个 taskId 都为空且不在集合里
   = 上次被打断，落 FAILED 并说明「录音本身完好」。云端两档的转码上传阶段
   落在同一个窗口里，这条顺带补上了那个既有缺口。

5. **模型体积是 1.5GB 不是「几百 MB」。** §6.2.1 的论证按「几百 MB」写的，
   实际 `Systran/faster-whisper-medium` 快照 1.46GB。论证的方向不变（反而更强），
   但界面上的下载提示要说真数：`sizeHint` 写「约 1.5GB」。
   依赖闭包另占 182MB（压缩后约 57MB 进安装包），大头是 onnxruntime 70MB 与 PyAV 44MB，
   两者都是 faster-whisper 的硬依赖，去不掉。
