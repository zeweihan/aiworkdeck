...
## 11. 决策（2026-08-08 已拍板，实施按此进行）

> 下面五条是提出时的备选与建议，**结论已由维护者确认**，逐条记在每条末尾。

**1. 国际站与中文产品的关系 → 站点缝先落，商业口径如实披露。**
桌面端仍是纯中文（`vue-i18n` 装了但 `src/` 下一处 `$t()` 都没有）。
国际站的服务条款与隐私政策**明文写出「桌面客户端当前界面语言为中文」**
（highlights 第 2 条 + 第一条正文 callout，并写明「若你不能接受，请不要购买」）。
桌面端 i18n 是独立的大工程，不在本次范围。

**2. 是否覆盖欧盟 → 覆盖，两条法务欠账本期一起清掉。**
自助注销（`/api/auth/delete` + 账户页危险区）与遥测 24 个月过期清理
（`purgeExpiredTelemetry` + admin 端点 + cron）本期都已实现。
不做地域限制——限制地域本身也要写进条款并做技术执行，比补齐这两条更麻烦。

**3. 国际站 registry 内容 → 只上免费与官方条目。**
`siteConfig.allowPaidSubmissions = false`，投稿表单在国际站隐藏定价字段。
付费投稿涉及美元结算与跨境付款给创作者，不在本期范围。

**4. 反馈收件箱 → 保持单一收件箱（addin.aiworkdeck.com），如实披露。**
国际站隐私政策第七条有一个专门的 callout：反馈内容会到达我们**在北京的服务器**，
中国法下的强制调取权范围更大、合同保障改变不了；因此反馈按 GDPR Art. 49(1)(a)
的明确同意处理、完全可选，并直接劝阻用户在反馈里粘贴客户材料。

**5. 国际站收款主体 → 香港实体 + Stripe（维护者已有实体与账户）。**
适用法为香港特别行政区法律，争议解决为 HKIAC 仲裁。
主体登记信息**不写死在代码里**，走 `NEXT_PUBLIC_AWD_INTL_ENTITY_*` 环境变量，
缺失时 `lib/site-config.ts` 在导入期直接抛错拒绝构建——
法律文本里的缔约主体是用户据以起诉的对象，编一个占位名字比留空危险得多。
