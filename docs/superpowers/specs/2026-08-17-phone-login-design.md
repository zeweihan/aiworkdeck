...
大陆站现有账号是用户名/密码的，直接关掉密码登录会把人全锁在门外。分三段：

1. **补绑期**：密码登录仍开，但登录后如果 `phone` 为空，强制走 `/sms/bind`（现成的）
   才能继续用。期间新注册只能用手机号。
2. **观察期**：看补绑覆盖率。未补绑的账号在登录页提示，并提供人工通道
   （管理员在后台代绑，用于手机号已停用的情况）。
3. **切断**：密码登录端点在 cn 站返回明确错误并指向手机号登录。

**每段的推进由配置开关控制，不靠改代码发版。** 切断这一步不可逆，要维护者点头。
...
| 用户收不到码 | 60 秒后可重发；连续 3 次失败提示改用人工通道，不要让人无限点 |
...
- 补绑期到切断之间留多久？建议至少两个发版周期。
- 号码停用、换号的人工通道由谁受理？（管理员后台代绑的操作留痕要求）
- 大陆站是否允许一个手机号对应多个账号？建议不允许（一号一账号，简化找回）。
