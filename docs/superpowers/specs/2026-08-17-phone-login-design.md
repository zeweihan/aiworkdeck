...
## 9.1 官网架构决策（2026-08-18 维护者拍板）

官网**有自己独立的一套鉴权**（`users.json` + username/email/password），与 `backend/`
的 Java 后端不是同一个系统，此前全仓没有任何手机号/短信痕迹。三个候选摆给维护者后
选定 **方案 A：官网自建短信**——官网继续持有账号体系，自己实现 dysmsapi 直签。

不选方案 B（转调 Java `sms-login`）的理由：官网账号是 Credits、支付、公开主页、
API key 的宿主。统一账号体系要么加一张 Java↔官网关联表（那还是两个账号），要么把
users.json 迁进 Java（动支付与主页）；还会让官网登录硬依赖北京 Java 云，而官网登录
背后就是付款。**B 的代价落在钱的路径上且无界，A 的代价（重写发码逻辑）有界且可测。**

国际站：端点同在但不启用短信，等 Twilio 开通后两站对称。

实现见官网仓 PR#66。

## 9.2 桌面端统一为账户登录（2026-08-18 维护者拍板）

**背景**：`AccountService` 的类注释写着「桌面端永远不需要登录（Spec §1）：与账户的
唯一连接方式是在设置页粘贴一枚 `awdk_` Key」。这条原则成立于官网只有「用户名+口令」
的年代——那时「在桌面端登录账户」意味着往桌面应用里输密码，比粘 Key 更差。
官网有了手机号验证码登录之后，这个前提没了。

**决策**：桌面端登录统一为账户登录。用户在桌面端填手机号+验证码，官网校验通过后
**服务端自动签发 `awdk_` Key**，用户再也看不到 Key。

`awdk_` Key **保留**为机器凭据不动——权益同步、平台 AI 取 key、
`accountFingerprintOrNull()`（换账号作废机器级缓存靠它）、流水都挂在它上面。
去掉的只是「去官网生成、复制、粘贴」这三步人肉操作。
现成的 `AwdkLoginService`（`POST /api/auth/awdk-login`）链路整条复用，只换输入。

**local-mode 的处置**：官方版本要求登录，`local-mode` **保留为配置项不删除**——
它被 37 个文件引用，其中四个是安全闸（`LocalModeAccessFilter`、`LocalModeLoopbackGuard`、
`PhoneLoginGuard`、`DataInitializer`），删它等于重构鉴权路径而产品收益为零。
改默认值（`application-desktop.yml: local-mode: false`）即可得到同样的产品结果，
且保住商业版/私有部署这条路不用 fork 分叉。开源项目里这个闸是**默认值不是 DRM**，
不投入做防篡改。

**这一条单独立项，不塞进登录 PR**，因为它必须与下面两件事一起设计：
1. **离线宽限**：不能把「需要登录」做成「需要联网才能开文档」。律师在飞机上、
   在律所内网必须照常开文档。抄 `license.json` 的 30 天离线宽限形态。
2. **存量工作区迁移**：现有用户的本地项目挂在免登本机身份上，翻成必须登录后要把
   那个工作区映射到新账户，否则用户升级完登录进来会觉得「项目没了」。
