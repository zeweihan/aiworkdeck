...
建仓前对工作区做体量预检（文件数 / 总字节）。超阈值**不默认建**，概览页给显式提示 + 「仍然开启」按钮。文件数阈值与导入上限同源（`MAX_IMPORT_ENTRIES=3000`，`LocalProjectService.java:40-41`）。**托管项目不做预检。**

##### iCloud 驱逐文件：比体量更严重，且体量阈值抓不到（2026-08-09 实测发现）

拿维护者一个真实案卷文件夹标定阈值时发现的，**这条改变了闸门 1 的设计**。

样本：某诉讼案卷，123 个文件 / 11 个目录 / 深度 3 / 无嵌套 git / 无符号链接。

| 指标 | 值 |
|---|---|
| `du -sh`（占盘） | **4.6 MB** |
| `stat st_size` 累加（逻辑） | **343.1 MB** |
| flags 带 `dataless` 的文件 | **110 / 123 个，占字节 98.7%** |

macOS「优化 Mac 储存空间」把 iCloud Drive 里的文件驱逐成占位符（`ls -lO` 显示 `compressed,dataless`，`st_blocks = 0`，`st_size` 仍是逻辑大小）。律师的案卷文件夹**天然就是这个形态**——几十兆的证据视频、扫描件 PDF，平时不看就被驱逐。

后果链：`git add "."` 要读每个文件的字节 → macOS 透明地把每个 dataless 文件**从 iCloud 下载回来** → 一次「只是打开了项目」的动作变成 338MB 下载 + 永久占盘（直接打掉用户自己开的「优化储存空间」），再加约等量的 git 对象（视频不压缩不 delta）。慢网络下就是长时间无进度地卡住。

**两条设计结论：**

1. **体量预检必须用逻辑大小（`st_size` 累加），不能用 `du` / 占盘。** 这个文件夹用 `du` 看是 4.6 MB，任何基于占盘的阈值都会放行——恰恰放行了最坏的情况。
2. **必须单独检测 dataless，不能靠字节阈值兜。** 343 MB 落在任何合理的字节阈值（1-2 GB）之下，光靠体量这个文件夹照样会被默认建仓。**只要工作区里存在 dataless 文件就不默认建仓**，转成显式提示：「这个文件夹里有 N 个文件还在 iCloud 上没下载（共 X MB），开启版本记录会把它们全部下载到本机。要继续吗？」

**检测手段（macOS，不触发materialize）**：`find "$root" -type f -flags +dataless -print -quit` 一次性问「有没有」，只读 flags 不读字节。JDK 的 `BasicFileAttributes` 不暴露 `st_flags`/`st_blocks`，所以这一条走 `ProcessBuilder` 调 `find` 比引 JNA 划算。Windows/Linux 侧的对应物是 OneDrive/Dropbox 的占位文件，检测 API 不同，**A 期只做 macOS 检测，其他平台退回纯逻辑字节阈值**。

> **另一条纪律**：这次没有对那个真实案卷跑 `git add` 计时——跑一次就等于把维护者 338MB 的 iCloud 数据拉回本机并永久占盘。`enableVersionRecording` 在大目录上的耗时仍标「未验证」，要测必须用一份**全部本地化的合成目录**，不能拿真实案卷试。
...
**Q1 · ~~体量预检的字节阈值定多少？~~ 已标定（2026-08-09）** —— 用维护者一个真实案卷样本测出：文件数沿用 3000；**字节阈值用逻辑大小、定 1 GB**。但真正的闸门是 **dataless 检测而不是体量**——那个样本 343 MB 逻辑 / 4.6 MB 占盘 / 98.7% 字节是 iCloud 占位符，任何合理的字节阈值都会放行它。详见 §3.3 闸门 1 的 iCloud 小节。**剩余未验证：`enableVersionRecording` 在大目录上的耗时**——要测必须造一份全部本地化的合成目录，不能拿真实案卷试（跑一次就把人家几百兆 iCloud 数据拉回本机并永久占盘）。
