...
### P4 — chrome 退场与逃生口（已完成）

**顺序上先补齐再隐藏。**理由是产品的：逃生开关应该是应急阀而不是常规路径——
律师插个脚注是日常动作，若要先去翻出「显示原生菜单栏」才能做，那不是变现代了
是变难用了；何况开关一开，用户要求删掉的那个 × 就一起回来。所以隐藏之前先补：

- **表格上下文组**（光标在表格里才出现）：上/下插入行、左/右插入列、删除行/列。
  行列号由 `get_ui_state` 新增的 `selection.cellName`（如 "B2"）算出——`table_*`
  原语收的是绝对行号列号，没有「当前位置」的概念。`position` 的语义是「插在该
  行/列之前」，所以「下方插入」= position+1。
- **脚注 / 页眉 / 页脚**：走已有原语。**没有尾注**——本引擎构建不支持
  （`IsEndnote` 抛 IllegalArgumentException），做不到的不放按钮。

隐藏本身：工具栏挂载后调 `set_chrome` 关掉 menubar / 14 条工具栏 / 状态栏 / 标尺。
只在 Writer 上做——本组件本来就只给 Writer 渲染，Calc/Impress 没有替代品，
藏了会剩一片空白。

**逃生开关：右侧「原生菜单」按钮**，一键把 LO 那套要回来。

真机实证的坑：**藏过全套之后单纯 `showElement()` 恢复不出来**。恢复路径必须
复核 + 重试——`showElement` → `createElement` + `showElement` → `setVisible(true)`
+ `showElement`，每步用 `isElementVisible` 复核。这条是兜底保证，不能碰运气。
