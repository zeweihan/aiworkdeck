...
> 状态（2026-08-04 二修）：PR#243 全量落地后，**维护者否决深色配色（要的是布局对齐，
> 不是配色对齐），深绿 chrome 已整体回退为原浅色体系**。保留的布局件：底部状态条、
> 外壳图标 SVG 化、插件广场 workbench 内嵌 tab、顶栏工具位结构。编辑器画布配色与
> FileTree/ChatInterface/左栏面板深色化全部还原。本文档中涉及深色 chrome 的段落
> 仅作方案存档，不再是现行状态。配色红线：外壳保持浅色。
