...
常规形态是**用 AI WorkDeck 账户登录**（手机号+验证码 / 邮箱+验证码 / 账号+口令，
其中邮箱验证码是国际站主路径，口令只剩存量账号还在用），与桌面版同一个账户；
私有部署与团队服务器可在「高级设置」里改用官网账户 Key（awdk_）或手工粘贴的设备令牌。
三条路换回来的都是 awdt_ 设备令牌（X-Session-Id 请求头携带），凭据本身不落盘。
对话复用主链路契约：
...
2. 设置页：手机号 + 验证码登录 → 直接进入对话视图（连接成功）。
   展开「高级设置」，填后端地址 + awdt_ 令牌 → 「测试连接」显示可访问项目数；
...
29. 对话进行中断网 10 秒再恢复 → 出现「连接中断，正在自动重连……」提示，
    ...
    （重连后按 run_state 兜底解锁）。
30. 设置页账户登录（主路径）：填手机号 → 「获取验证码」→ 按钮转 60 秒倒计时、收到短信；
    故意填错验证码 → 原样显示官网文案「验证码错误或已过期」，不落任何令牌；
    填对 → 直接进入对话视图，重开任务窗格后连接仍在（保存的是 awdt_，凭据本身不落盘）。
    切到「邮箱」tab：默认就是邮箱+验证码（国际站主路径），「获取验证码」同样转 60 秒倒计时、
    收到邮件后填码能进对话视图；再点「改用口令登录」切到邮箱+口令走一遍，同样两条判据。
    切 tab 后「获取验证码」仍能弹出人机验证滑块（控件节点常驻，不随 tab 被拆掉）。
31. 高级设置「用 Key 连接」：粘贴有效 awdk_ Key → 换取 awdt_ 令牌并直接进入对话视图。
    对未开启账户桥接的服务器 → 提示改用高级设置里的 Key 或设备令牌。
