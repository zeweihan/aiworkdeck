...
**项目名默认随团队统计上传，但第一次上传之前会先问你一次。** 项目名真正要出本机的那一刻，应用会把将要上传的项目名原样列出来，说明它们会出现在团队看板的哪里、谁看得到，由你当场选择「同意上传」或「只传匿名编号」；没有做出选择之前，这段统计留在本机不上传。选「只传匿名编号」之后项目名一律不出本机，统计其余部分照常上报，这个选择存在你自己的机器上，随时可以在同一处改回来。

另外，团队管理者可以在「团队设置」里关闭「共享项目名」；关闭后全体成员的上报只带匿名编号，团队面板只显示匿名编号（管理者仍可以给编号起一个团队内部叫法，这个叫法存在服务器上，与你机器上的真实项目名无关）——这是整个团队的设置，会影响全体成员。
