...
def _should_force_cloud() -> bool:
    ...
    try:
        ...
        if current_app and hasattr(current_app, "config"):
            v = current_app.config.get("MINERU_FORCE_CLOUD")
            if v is not None:
                return _truthy(v)
    except RuntimeError:
    ...
    def _check_local_service(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"{self.mineru_local_url}/docs",
                timeout=5
            )
            if response.status_code == 200:
                logger.info(f"Local MinerU service (official mineru-api) available at {self.mineru_local_url}")
                self._use_local_service = True
                return True
    ...
    def _parse_with_local_service(self, file_path: str, filename: str) -> tuple[Optional[str], Optional[str], Optional[str], Optional[str], int]:
        ...
        try:
            ...
            with open(file_path, 'rb') as f:
                ...
                data = {
                    'backend': 'pipeline',  # Use pipeline backend for CPU mode
    ...
    def parse_file(self, file_path: str, filename: str) -> tuple[Optional[str], Optional[str], Optional[str], Optional[str], int]:
        ...
        try:
            ...
            if _should_force_cloud():
                ...
                logger.info("Using local MinerU service...")
                result = self._parse_with_local_service(file_path, filename)
